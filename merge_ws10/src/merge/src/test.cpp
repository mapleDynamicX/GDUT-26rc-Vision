...
void test4()
{
    ...
    while(ros::ok())
    {
        Ten::Ten_serial::GetInstance().serial_send(arr, 1, sizeof(float)*4);
...
void test10()
{
    Eigen::Matrix3f R = Ten::createRotationMatrix(- M_PI / 2.0, - M_PI / 2.0, 0);
    std::cout<< R << std::endl;
}
...
void test_yolo2()
{
    //创建检测器
    ros::NodeHandle nh("~");
    Ten::Ten_yolo detector("/home/maple/study2/merge_ws10/src/merge/model/houli_1023_v5s/best", "cpu", 0);
    Ten::Ten_camera& camera =  Ten::Ten_camera::GetInstance();
