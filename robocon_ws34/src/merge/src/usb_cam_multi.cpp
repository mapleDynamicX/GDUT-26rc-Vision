...
// 重置指定摄像头
// 全量重置所有摄像头
bool Ten_usb_cam_multi::reset_camera(std::vector<int> dev_idxs, size_t w, size_t h, size_t fps)
{
    std::lock_guard<std::mutex> lock(read_mtx_);

    // 1. 关闭并释放所有已打开的旧摄像头，清空映射表
    for (auto& pair : caps_)
    {
        if (pair.second.isOpened())
            pair.second.release();
    ...
    // 2. 更新类内全局参数与设备列表
    dev_idxs_ = std::move(dev_idxs);
    w_ = w;
    h_ = h;
    fps_ = fps;

    // 3. 复用原有初始化逻辑，逐台初始化新列表中的摄像头
    bool all_success = true;
    for (int idx : dev_idxs_)
    {
        if (!init_single_camera(idx))
            all_success = false;
    }
