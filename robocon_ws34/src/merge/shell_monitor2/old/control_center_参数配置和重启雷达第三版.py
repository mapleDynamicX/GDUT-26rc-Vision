...
class TouchscreenUI:
    ...
    def build_left_panel(self, parent, col):
        ...
        self.btn_grid_view = tk.Button(title_bar, text="梅林输入区", font=('DejaVu Sans', 30, 'bold'),
                                       command=lambda: self.switch_left_view('grid'))
        self.btn_grid_view.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(0, 5))
        self.btn_script_view = tk.Button(title_bar, text="脚本运行区", font=('DejaVu Sans', 30, 'bold'),
                                         command=lambda: self.switch_left_view('script'))
        self.btn_script_view.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=(5, 0))
        # 新增参数配置Tab按钮
        self.btn_param_view = tk.Button(title_bar, text="参数配置区", font=('DejaVu Sans', 30, 'bold'),
    ...
    def build_param_block_content(self, parent):
        ...
        # 垂直滚动条
        scrollbar = tk.Scrollbar(wrap_frame, orient=tk.VERTICAL)
        ...
        self.param_block_height = 300  # 统一设定为合理的固定高度
    ...
    def build_grid_block_content(self, parent):
        ...
        tk.Label(table_frame, text="R1\\R2", bg="#2d2d2d", fg="black", font=('DejaVu Sans', 20, 'bold')).grid(row=0,
                                                                                                              column=0,
                                                                                                              sticky='nsew',
                                                                                                              padx=1,
                                                                                                              pady=1)
        for col_idx, r2_cnt in enumerate([2, 3, 4], start=1):
            tk.Label(table_frame, text=f"{r2_cnt}个", bg="#2d2d2d", fg="black", font=('DejaVu Sans', 20, 'bold')).grid(
                row=0, column=col_idx, sticky='nsew', padx=1, pady=1)
        for row_idx, r1_cnt in enumerate([1, 2, 3], start=1):
            tk.Label(table_frame, text=f"{r1_cnt}个", bg="#2d2d2d", fg="black", font=('DejaVu Sans', 20, 'bold')).grid(
