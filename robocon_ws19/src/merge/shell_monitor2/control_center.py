...
class TouchscreenUI:
    ...
    def draw_parsed_paths_graphics(self):
        """核心绘图：在画布上绘制 R2 蓝色路径线，以及 R1/R2 的动作方块右侧目标框"""
        ...
        # 2. 绘制 R2 夹取的方块（蓝色框住右边字体区域）
        # 字体设定在 x + width*0.9，我们把框画在 0.65 到 0.95 的范围
        for idx in self.r2_targets:
            x1, y1 = self.get_coords_from_index(idx)
            bx1, by1 = x1 + self.cell_w * 0.65, y1 + self.cell_h * 0.25
            bx2, by2 = x1 + self.cell_w * 0.95, y1 + self.cell_h * 0.75
            self.canvas.create_rectangle(bx1, by1, bx2, by2, outline="#00ffff", width=box_line_w)

        # 3. 绘制 R1 夹取的方块（橙色框住右边字体区域）
        # R1 的框略微放大一点点 (0.6 到 0.97)，避免如果万一 R1、R2 在同格时线框重叠
        for idx in self.r1_targets:
            x1, y1 = self.get_coords_from_index(idx)
            bx1, by1 = x1 + self.cell_w * 0.60, y1 + self.cell_h * 0.20
            bx2, by2 = x1 + self.cell_w * 0.98, y1 + self.cell_h * 0.80
            self.canvas.create_rectangle(bx1, by1, bx2, by2, outline="#ffb86c", width=box_line_w)
