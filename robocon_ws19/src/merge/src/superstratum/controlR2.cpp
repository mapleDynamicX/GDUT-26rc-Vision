...
namespace Ten
{
    ...
    void response(int data)
    {
        ...
        if (!int_pub)
        {
            ros::NodeHandle nh("/script");
            int_pub = nh.advertise<std_msgs::Int32>("/response", 10);
        }
    ...
    void pathCallback(const std_msgs::Int32::ConstPtr& msg)
    {
        ...
        if(receive != last)
        {
            ...
            if(length)
            {
                ...
                while(Ten::_MAP_FLAG_.read_flag())
                {
    ...
    void script_control()
    {
        ros::NodeHandle nh("/script");
        // 订阅int话题
        ros::Subscriber send_path = nh.subscribe("/send_path", 3, pathCallback);
