...
class MonitorStandalone:
    ...
    def _create_single_lower_widget(self, parent_container, name):
        """渲染单个下位机数据块"""
        block = tk.Frame(parent_container, bg="#383838", bd=2, relief=tk.RAISED, height=80)
        block.pack_propagate(False)

        tk.Label(block, text=name, bg="#383838", fg="white", font=("DejaVu Sans", 10, "bold")).pack(anchor="w",
                                                                                                    pady=1)
        ...
        data_label.pack(fill=tk.BOTH, expand=True, pady=1)
