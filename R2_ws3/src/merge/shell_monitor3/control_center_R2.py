...
class TouchscreenUI:
    def __init__(self, master):
        ...
        win_height = int(screen_height * 0.79)
        # 计算居中时的左上角坐标
        ...
        self.main_frame.columnconfigure(0, weight=20)  # 12宫格区
        ...
        # --- 修改结束 ---
        # --- 新增：红蓝半场初始化变量 ---
        self.is_red_court = False # 仅初始化值，不是默认值
        self.bg_red = "#FFADA3"  # 偏暗红色调
        self.bg_blue = "#BBEFF2"  # 偏暗蓝色调

        # --- 赛制与约束条件提取 ---
        self.is_challenge_mode = False  # 默认为正赛
        self.regular_qty_constraints = {0: 4, 1: 3, 2: 4, 3: 1}  # 正赛数量约束
        self.challenge_qty_constraints = {0: 7, 1: 2, 2: 2, 3: 1}  # 挑战赛数量约束

        self.regular_databases = {
            (1, 2): r"./official_path/path_12.txt", (1, 3): r"./official_path/path_13.txt",
            (1, 4): r"./official_path/path_14.txt",
            (2, 2): r"./official_path/path_22.txt", (2, 3): r"./official_path/path_23.txt",
            (2, 4): r"./official_path/path_24.txt",
            (3, 2): r"./official_path/path_32.txt", (3, 3): r"./official_path/path_33.txt",
            (3, 4): r"./official_path/path_34.txt",
        }
        self.challenge_databases = {
            (1, 1): r"./challenge_path/path_11.txt", (1, 2): r"./challenge_path/path_12.txt",
            (2, 1): r"./challenge_path/path_21.txt", (2, 2): r"./challenge_path/path_22.txt",
        }
        self.databases = self.regular_databases
        # ===================================================================
        ...
        # ===================== 修改：设备自检区模块变量（统一话题，独立开关映射） =====================
        self.check_tasks_meta = {
            "side_cam": {"name": "两侧相机自检", "vals": [1, 2]},
            "front_cam": {"name": "前向相机自检", "vals": [3, 4]},
            "lidar": {"name": "雷达自检", "vals": [5, 6]}
        }
        # 状态机：0(初始空闲), 1(开指令发送中), 2(已开启/自检中), 3(关指令发送中), 4(已关完成), 5(开排队中), 6(关排队中)
        self.check_states = {k: 0 for k in self.check_tasks_meta.keys()}
        self.check_ui_elements = {}

        # 核心：开关变量彻底分开，分别记录每个设备上一次发送的开值和关值
        self.check_last_on_values = {k: None for k in self.check_tasks_meta.keys()}
        self.check_last_off_values = {k: None for k in self.check_tasks_meta.keys()}

        # 所有设备统一共用这一个话题和发布器
        self.check_pub = rospy.Publisher('/scripts/check', Int32, queue_size=1)
        self.global_check_off_pub = rospy.Publisher('/scripts/global_check_off', Int32, queue_size=1)

        # 新增yaw测试相关参数
        self.yaw_topic_map = {1: "/R1/odom", 2: "/R2/odom"}  # 机器人ID对应话题
        self.current_yaw_robot = 2  # 默认选择R2
        self.yaw_record_count = 10  # 连续采样的点数变量
        # ===================================================================================

        # === 新增：自动识别并关联参数区的红蓝半场，统一默认值为红半区 ===
        self.court_param_key = None
        self.match_param_key = None  # 新增：赛制选择参数的 key
        for k, v in self.params_meta.items():
            if any(kw in v["name"] for kw in ["半场", "红蓝", "分场", "场地"]):
                self.court_param_key = k
            if any(kw in v["name"] for kw in ["赛制", "比赛模式", "挑战赛", "正赛"]):
                self.match_param_key = k
    ...
    def run_data_monitor_script(self):
        """点击【启动数据监视】按钮时，打开独立终端运行 sh 脚本"""
        sh_script_path = "./watch_window.sh"

        try:
            # 使用 gnome-terminal 打开新窗口运行脚本
            # 加入 exec bash 保证脚本执行完毕或报错后终端不会立刻闪退关闭
            cmd = ["gnome-terminal", "--", "bash", "-c", f"bash {sh_script_path}"]

            # 使用 preexec_fn=os.setsid 确保它在独立的进程组中运行
            subprocess.Popen(cmd, preexec_fn=os.setsid)

            rospy.loginfo(f"数据监视脚本已在独立终端启动: {sh_script_path}")

        except Exception as e:
            rospy.logerr(f"启动数据监视脚本失败: {e}")
    ...
    def build_ros_status_bar(self, parent):
        ...
        for key, var in self.ros_status_vars.items():
            lbl = tk.Label(inner_frame, textvariable=var, bg="#2d2d2d",
                           fg="#f1fa8c", font=('DejaVu Sans', 10, 'bold'))
    ...
    def load_and_create_script_buttons(self):
        ...
        for s in scripts:
            ...
            btn = tk.Button(self.script_container, text=name, bg="#44475a", fg="white",
                            font=('DejaVu Sans', 14), height=2, anchor="w", padx=8)
            btn.pack(fill=tk.X, pady=2, expand=True)
            # 解决鼠标悬停在按钮上时无法滚动的问题
    ...
    def _load_params_json(self):
        ...
        try:
            ...
            for k, v in raw_cfg.items():
                param_type = v.get("type", "choose")  # 默认值为 choose 兼容老配置
                name = v.get("name")

                if param_type == "choose":
                    rng = v.get("range")
                    map_dict = v.get("map", {})
    ...
    def toggle_court_side(self):
        """处理红蓝半场切换按钮逻辑"""
        if not self._is_interaction_allowed(): return  # 新增拦截
    ...
    def _build_plan_table(self):
        """动态构建方案选择表格，适配正赛与挑战赛的不同维度"""
        for widget in self.table_frame.winfo_children():
            widget.destroy()
        self.plan_buttons.clear()

        r1_range = [1, 2] if self.is_challenge_mode else [1, 2, 3]
        r2_range = [1, 2] if self.is_challenge_mode else [2, 3, 4]

        # 重新配置权重
        for i in range(len(r2_range) + 1):
            self.table_frame.columnconfigure(i, weight=1)
        for i in range(len(r1_range) + 1):
            self.table_frame.rowconfigure(i, weight=1)

        bg_transparent = self.table_frame['bg']
        tk.Label(self.table_frame, text="R1\\R2", fg="black", bg=bg_transparent, font=('DejaVu Sans', 9, 'bold'),
                 width=7).grid(row=0, column=0, sticky='nsew', pady=1)

        for col_idx, r2_cnt in enumerate(r2_range, start=1):
            tk.Label(self.table_frame, text=f"{r2_cnt}个", fg="black", bg=bg_transparent,
                     font=('DejaVu Sans', 8, 'bold')).grid(row=0, column=col_idx, sticky='nsew', padx=1, pady=1)

        for row_idx, r1_cnt in enumerate(r1_range, start=1):
            tk.Label(self.table_frame, text=f"{r1_cnt}个", fg="black", bg=bg_transparent,
                     font=('DejaVu Sans', 8, 'bold')).grid(row=row_idx, column=0, sticky='nsew', padx=1, pady=1)
            for col_idx, r2_cnt in enumerate(r2_range, start=1):
                lbl = tk.Label(self.table_frame, text="-", fg="white", width=5, bg=bg_transparent,
    ...
    def toggle_match_mode(self):
        """点击按钮切换正赛/挑战赛"""
        if not self._is_interaction_allowed(): return
        self.is_challenge_mode = not self.is_challenge_mode
        self._update_match_mode_state()

        # 联动更新参数选择区
        if getattr(self, 'match_param_key', None):
            for val, text in self.params_meta[self.match_param_key]["map"].items():
                is_challenge_text = "一区" in text or "三区" in text
                if (self.is_challenge_mode and is_challenge_text) or (not self.is_challenge_mode and "正赛" in text):
                    self.params_dict[self.match_param_key] = val
                    for v, btn in self.param_blocks[self.match_param_key].items():
                        if v == val:
                            btn.config(bg="#f1fa8c", fg="black")
                        else:
                            btn.config(bg="#44475a", fg="white")
    ...
    def _update_match_mode_state(self):
        """根据当前的赛制状态刷新底层数据和 UI"""
        if self.is_challenge_mode:
            self.match_mode_var.set("挑\n战\n赛")
            self.match_mode_toggle_btn.config(text="切换正赛")
            self.databases = self.challenge_databases
        else:
            self.match_mode_var.set("正\n赛")
            self.match_mode_toggle_btn.config(text="切挑战赛")
            self.databases = self.regular_databases

        self._build_plan_table()
        self.check_map_validity()
        self.redraw_canvas()
    ...
    def update_court_ui(self):
        ...
        btn_text = "切蓝场" if self.is_red_court else "切红场"
    ...
    def setup_styles(self):
        """配置适配800x600尺寸的 ttk 样式"""
        style = ttk.Style()
        style.theme_use('clam')
        # 状态按钮样式
        style.configure('Status.TButton', font=('DejaVu Sans', 10, 'bold'), padding=8)
        # ROS 命令按钮样式
        style.configure('Cmd.TButton', font=('DejaVu Sans', 11, 'bold'), padding=10)
        # 标题标签样式
        style.configure('Header.TLabel', background='#1e1e1e', foreground='#ffffff', font=('DejaVu Sans', 11, 'bold'))
    ...
    def build_left_panel(self, parent, col):
        ...
        left_panel.grid(row=0, column=col, sticky='nsew', padx=5, pady=5)
        # 标题栏兼切换按钮区 (放大按钮，充当 Tab 标题)
        title_bar = tk.Frame(left_panel, bg='#2d2d2d')
        title_bar.pack(fill=tk.X, pady=5, padx=5)
        self.btn_grid_view = tk.Button(title_bar, text="梅林输入区", font=('DejaVu Sans', 20, 'bold'),
                                       command=lambda: self.switch_left_view('grid'))
        self.btn_grid_view.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=1)
        # 新增参数配置Tab按钮
        self.btn_param_view = tk.Button(title_bar, text="参数配置区", font=('DejaVu Sans', 20, 'bold'),
                                        command=lambda: self.switch_left_view('param'))
        self.btn_param_view.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=1)
        self.btn_check_view = tk.Button(title_bar, text="设备自检区", font=('DejaVu Sans', 20, 'bold'),
                                        command=lambda: self.switch_left_view('check'))
        self.btn_check_view.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=1)
    ...
    def switch_left_view(self, view_name):
        ...
        if view_name == 'grid':
            ...
            # ========== 修复：切换到自检区后强制重排，确保yaw区块立即显示 ==========
            self.master.update_idletasks()  # 先等待Tkinter完成页面渲染，获取真实画布宽度
            self._on_check_canvas_resize()  # 手动执行一次布局重排
    ...
    def build_param_block_content(self, parent):
        ...
        wrap_frame.pack(fill=tk.BOTH, expand=True, padx=1, pady=1)
        ...
        # ========== 核心修改：缩放适应800x600 ==========
        self.param_block_width = 1000
        self.param_block_height = 120
        self.param_block_gap = 5
        # ====================================================================
    ...
    def _create_single_param_widget(self, parent_container, param_key):
        ...
        name = meta["name"]
        param_type = meta.get("type", "choose")
        ...
        # 参数内容容器 (替代原本的 btn_wrap)
        content_wrap = tk.Frame(block, bg="#383838")
        content_wrap.pack(anchor="w", padx=5, pady=4)
        ...
        if param_type == "choose":
            ...
            for val in range(minv, maxv + 1):
                ...
                btn = tk.Button(
                    content_wrap, text=txt, font=("DejaVu Sans", btn_font_size, "bold"),
                    height=15,
            ...
            # 使用 StringVar 绑定 Label 实现数值动态刷新
            val_var = tk.StringVar(value=str(self.params_dict[param_key]))

            btn_minus = tk.Button(
                content_wrap, text="-", font=("DejaVu Sans", btn_font_size, "bold"),
                height=15, width=2,
                command=lambda k=param_key, s=-scale, v=val_var: self._on_number_param_change(k, s, v)
            )
            btn_minus.pack(side=tk.LEFT, padx=3)

            val_label = tk.Label(
                content_wrap, textvariable=val_var, bg="#383838", fg="#f1fa8c",
                font=("DejaVu Sans", btn_font_size, "bold"), width=4
            )
            ...
            unit_label = tk.Label(
                content_wrap, text=unit, bg="#383838", fg="white",
                font=("DejaVu Sans", name_font_size, "bold")
            )
        ...
        content_wrap.bind("<Button-4>", _scroll)
        content_wrap.bind("<Button-5>", _scroll)

        # 遍历所有子组件绑定滚轮，防止悬停在内部 Label/Button 上时滚动失效
        for child in content_wrap.winfo_children():
    ...
    def _on_param_select(self, param_key, val):
        ...
        # 联动更新梅林输入区正赛/挑战赛
        if getattr(self, 'match_param_key', None) and param_key == self.match_param_key:
            text = self.params_meta[param_key]["map"].get(val, "")
            if "一区" in text or "三区" in text:
    ...
    def _on_param_canvas_resize(self, event):
        ...
        new_block_width = max(150, (canvas_width // cols) - 10)

        # 清空原有布局
        for widget in self.param_block_widgets:
            widget.grid_forget()
        # 新增：清空yaw块布局
        if hasattr(self, 'yaw_block_widget'):
            self.yaw_block_widget.grid_forget()
    ...
    def _on_number_param_change(self, param_key, delta, val_var):
        """处理 number 类型参数的加减逻辑"""
        current_val = self.params_dict[param_key]
        new_val = current_val + delta

        # 处理浮点数计算引发的精度问题 (例如 0.1 + 0.2 = 0.30000000000000004)
        if isinstance(new_val, float):
            new_val = round(new_val, 4)
            # 如果小数部分为 0，则自动转回整数，保持显示美观
            if new_val.is_integer():
                new_val = int(new_val)

        # ========== 新增：限幅检查 ==========
        limit = self.params_meta[param_key].get("limit")
        if limit and isinstance(limit, list) and len(limit) >= 2:
            min_val, max_val = limit[0], limit[1]
            if new_val < min_val or new_val > max_val:
                rospy.logwarn(f"参数 [{param_key}] 修改被拒绝：目标值 {new_val} 超出限制范围 [{min_val}, {max_val}]")
                return  # 拦截后续的 UI 更新和变量修改
        # ==================================

        self.params_dict[param_key] = new_val
        val_var.set(str(new_val))
    ...
    def build_grid_block_content(self, parent):
        ...
        constraint_frame.pack(fill=tk.X)
        ...
        self.pos_constraint_label = tk.Label(constraint_frame, textvariable=self.pos_constraint_var, fg="#ff5555",
                                             bg="#2d2d2d", font=('DejaVu Sans', 8, 'bold'))
        ...
        self.split_container.pack(expand=True, fill=tk.BOTH)
        ...
        btn_frame.pack(side=tk.RIGHT, fill=tk.BOTH, padx=8)
        self.states_config = [
        ...
        self.match_mode_var = tk.StringVar(value="正\n赛")
        ttk.Label(left_frame, textvariable=self.match_mode_var, style='Header.TLabel',
                  font=('DejaVu Sans', 12, 'bold'),
                  justify=tk.CENTER).pack(side=tk.LEFT)

        # 新增：用于在表格和手动工具之间切换的父容器
        self.left_frame_container = tk.Frame(left_frame, bg='#2d2d2d')
        self.left_frame_container.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # ------------------ 表格区域 ------------------
        self.table_frame = tk.Frame(self.left_frame_container, bg='#2d2d2d')
        self.table_frame.pack(fill=tk.BOTH, expand=True)
        self._build_plan_table()

        # ------------------ 新增：手动路径工具栏 ------------------
        self.manual_tools_frame = tk.Frame(self.left_frame_container, bg='#2d2d2d')
        self.manual_edit_type = "path"
        self.is_manual_mode = False

        self.manual_btn_path = tk.Button(self.manual_tools_frame, text="路径", font=('DejaVu Sans', 14, 'bold'),
                                         command=lambda: self.set_manual_edit_type("path"))
        self.manual_btn_r1 = tk.Button(self.manual_tools_frame, text="R1", font=('DejaVu Sans', 14, 'bold'),
                                       command=lambda: self.set_manual_edit_type("r1"))
        self.manual_btn_r2 = tk.Button(self.manual_tools_frame, text="R2", font=('DejaVu Sans', 14, 'bold'),
                                       command=lambda: self.set_manual_edit_type("r2"))

        self.manual_btn_path.pack(side=tk.LEFT, expand=True, fill=tk.BOTH, padx=2)
        self.manual_btn_r1.pack(side=tk.LEFT, expand=True, fill=tk.BOTH, padx=2)
        self.manual_btn_r2.pack(side=tk.LEFT, expand=True, fill=tk.BOTH, padx=2)
        ...
        # --- 修改：将红蓝场切换和手动路径切换按钮放在一个横向容器中 ---
        btn_top_frame = tk.Frame(right_frame, bg='#2d2d2d')
        btn_top_frame.grid(row=0, column=0, sticky='ne')

        self.manual_mode_btn = tk.Button(btn_top_frame, text="手动路径", bg="#44475a", fg="white", height=2,
                                         font=('DejaVu Sans', 14, 'bold'), command=self.toggle_manual_mode)
        self.manual_mode_btn.pack(side=tk.LEFT)

        # 修改：将原本宽度 30 缩小，给新按钮腾位置
        self.court_toggle_btn = tk.Button(btn_top_frame, text="切蓝场", font=('DejaVu Sans', 14, 'bold'), height=2,
                                           command=self.toggle_court_side)
        self.court_toggle_btn.pack(side=tk.LEFT, padx=2)

        # 新增：赛制切换按钮
        self.match_mode_toggle_btn = tk.Button(btn_top_frame, text="切挑战赛", font=('DejaVu Sans', 14, 'bold'), height=2,
                                               command=self.toggle_match_mode)
        self.match_mode_toggle_btn.pack(side=tk.LEFT, padx=2)

        info_container = tk.Frame(right_frame, bg='#2d2d2d')
        info_container.grid(row=1, column=0, sticky='w')
        self.r2_path_var = tk.StringVar(value="R2：暂无")
        self.r1_path_var = tk.StringVar(value="R1：暂无")
        self.r2_path_label = tk.Label(info_container, textvariable=self.r2_path_var, fg="#000000", bg="#2d2d2d",
                 font=('DejaVu Sans', 12, 'bold'), anchor=tk.W, width=35, justify=tk.LEFT, wraplength=200)
        self.r2_path_label.pack(fill=tk.X)
        self.r1_path_label = tk.Label(info_container, textvariable=self.r1_path_var, fg="#0021FA", bg="#2d2d2d",
                 font=('DejaVu Sans', 12, 'bold'), anchor=tk.W, width=35, justify=tk.LEFT, wraplength=1200)
        self.r1_path_label.pack(fill=tk.X)
    ...
    def check_map_validity(self):
        ...
        for state in self.grid_states.values():
            counts[state] += 1
        # 数量校验
        constraints = self.challenge_qty_constraints if self.is_challenge_mode else self.regular_qty_constraints
        qty_ok = (counts[0] == constraints[0] and
                  counts[1] == constraints[1] and
                  counts[2] == constraints[2] and
                  counts[3] == constraints[3])
        ...
        if is_valid:
            ...
            self.r2_path_var.set("R2：暂无")
            self.r1_path_var.set("R1：暂无")
    ...
    def toggle_manual_mode(self):
        ...
        if self.is_manual_mode:
            self.manual_mode_btn.config(bg="#f1fa8c", fg="black")
            self.table_frame.pack_forget()
            self.manual_tools_frame.pack(fill=tk.BOTH, expand=True)
            self.set_manual_edit_type("path")  # 默认选中路径
            self.update_ros_status("path", "手动路径：未发送", "#f1fa8c")
        else:
            self.manual_mode_btn.config(bg="#44475a", fg="white")
            self.manual_tools_frame.pack_forget()
            self.table_frame.pack(fill=tk.BOTH, expand=True)
            if hasattr(self, 'r1_path_label'):
                self.r1_path_label.config(fg="#0021FA")  # 退出手动模式时恢复 R1 标签为蓝色
            if hasattr(self, 'r2_path_label'):
                self.r2_path_label.config(fg="#000000")  # 退出手动模式时恢复 R2 标签为原色
            self.check_map_validity()  # 退出时恢复算法生成的地图状态
        self.redraw_canvas()

    def set_manual_edit_type(self, edit_type):
        """切换手动模式下的笔刷状态 (路径/R1/R2)"""
        self.manual_edit_type = edit_type
        # 重置按钮高亮
        for btn in (self.manual_btn_path, self.manual_btn_r1, self.manual_btn_r2):
            btn.config(bg="#44475a", fg="white")
        if edit_type == "path":
            self.manual_btn_path.config(bg="#f1fa8c", fg="black")
        elif edit_type == "r1":
            self.manual_btn_r1.config(bg="#f1fa8c", fg="black")
        elif edit_type == "r2":
            self.manual_btn_r2.config(bg="#f1fa8c", fg="black")

    def _is_adjacent(self, idx1, idx2):
        """判断两格子在绝对拓扑上是否相邻（上下左右）"""
        row1, col1 = idx1 // 3, idx1 % 3
        row2, col2 = idx2 // 3, idx2 % 3
        return abs(row1 - row2) + abs(col1 - col2) == 1

    def _validate_manual_r2_targets(self):
        """校验手动模式下的 R2 方块是否符合位置约束"""
        self.invalid_r2_targets = []
        for target in getattr(self, 'r2_targets', []):
            # 1, 2, 3 上的 R2 方块无脑可选 (对应 idx 0, 1, 2)
            if target in [0, 1, 2]:
                continue
            # 在路径上的方块符合约束
            if target in self.r2_robot_path:
                continue
            # 在路径两侧（与路径拓扑相邻）的方块符合约束
            if any(self._is_adjacent(target, p) for p in self.r2_robot_path):
                continue
            # 都不满足则判定为不符合约束
            self.invalid_r2_targets.append(target)

        return len(self.invalid_r2_targets) == 0

    def _update_manual_text(self):
        """刷新UI上R1和R2文本的显示"""
        if getattr(self, 'is_manual_mode', False):
            # ================= 新增：检查路径上是否有 R1 未被夹取 =================
            r1_invalid = any(self.grid_states.get(p) == 1 and p not in getattr(self, 'r1_targets', []) for p in
                             getattr(self, 'r2_robot_path', []))

            if r1_invalid:
                self.r1_path_var.set("路径上有R1未夹取")
                if hasattr(self, 'r1_path_label'):
                    self.r1_path_label.config(fg="#ff0000")  # 不满足约束时变红
            else:
                r1_str = "→".join([str(i + 1) for i in self.r1_targets]) if self.r1_targets else "未选择"
                self.r1_path_var.set(f"R1 夹取：{r1_str}")
                if hasattr(self, 'r1_path_label'):
                    self.r1_path_label.config(fg="#0021FA")  # 满足约束时恢复默认蓝色
            # ======================================================================

            # ================= 新增：R2 文本路径格式化与约束显示 =================
            r2_invalid = any(self.grid_states.get(p) == 2 and p not in getattr(self, 'r2_targets', []) for p in
                             getattr(self, 'r2_robot_path', []))

            if r2_invalid:
                self.r2_path_var.set("路径上有R2未夹取")
                if hasattr(self, 'r2_path_label'):
                    self.r2_path_label.config(fg="#ff0000")  # 不满足约束变红
            else:
                if not self._validate_manual_r2_targets():
                    # 约束不满足，显示红色警告
                    r2_display_text = "选中R2无法夹取"
                    self.r2_path_label.config(fg="#ff0000") # 变红
                else:
                    # 约束满足，恢复正常显示，解析步骤
                    self.r2_path_label.config(fg="#000000") # 颜色恢复

                    # 核心修改：在所有转为 str 或直接推入 steps 的地方将序号 + 1
                    steps = []
                    # 1. 无脑前置处理 1,2,3 位置 (内部索引 0, 1, 2)
                    for t in [0, 1, 2]:
                        if t in getattr(self, 'r2_targets', []):
                            steps.append(f"({t + 1})")  # 修改点：t -> t + 1

                    # 2. 遍历路径，分配剩余的目标方块
                    grabbed = set([0, 1, 2])
                    for p in getattr(self, 'r2_robot_path', []):
                        step_str = str(p + 1)  # 修改点：p -> p + 1
                        current_grabs = []

                        # 寻找属于该路径点的目标 (在同一格或相邻)，且未被夹取过
                        for t in getattr(self, 'r2_targets', []):
                            if t not in grabbed and (t == p or self._is_adjacent(p, t)):
                                current_grabs.append(t + 1)  # 修改点：t -> t + 1
                                grabbed.add(t)

                        # 存在要夹取的方块则拼装 (例如 "(5 9)")
                        if current_grabs:
                            grabs_str = " ".join(map(str, current_grabs))
                            step_str += f"({grabs_str})"

                        steps.append(step_str)

                    r2_display_text = " ".join(steps)

                self.r2_path_var.set(r2_display_text)
                # if hasattr(self, 'r2_path_label'):
                #     self.r2_path_label.config(fg="#000000")  # 满足约束时恢复默认颜色


    def _is_interaction_allowed(self):
        """统一拦截器：返回 True 表示允许交互，False 表示被手动模式拦截"""
        if getattr(self, 'is_manual_mode', False):
            # 可以在这里添加一些提示，或者保持静默
            self.update_ros_status("manual", "手动模式下禁用此操作", "#ff0000")
            return False
        return True
    ...
    def update_parsed_path_display(self):
        ...
        # --- 新增这部分：如果当前已有选中的有效方案，缩放窗口时重新触发高亮以恢复 UI ---
            ...
            # 9种情况全无解
            self.selected_plan_key = None
            self.r2_path_var.set("R2：无解")
            self.r1_path_var.set("R1：无解")
            self.r2_robot_path.clear()
            self.r2_targets.clear()
            self.r1_targets.clear()
            self.redraw_canvas()
    ...
    def on_plan_selected(self, r1, r2):
        ...
        self.r2_path_var.set(f"R2：{plan['r2_str']}")
        self.r1_path_var.set(f"R1：{plan['r1_str']}")
        self.redraw_canvas()
    ...
    def draw_grids(self):
        ...
        for idx in range(12):
            ...
            # 绘制左下角序号
            num_font_size = max(18, int(self.cell_h * 0.06 + self.cell_h * 0.06))
    ...
    def on_canvas_click(self, event):
        ...
        if not (0 <= idx <= 11): return

        # --- 新增：手动模式下的交互逻辑 ---
        if getattr(self, 'is_manual_mode', False):
            if self.manual_edit_type == "path":
                # ================= 新增：F方块约束 =================
                # 不能经过F方块（假设 grid_states 值为 3 代表 F 方块，请根据实际情况调整）
                if self.grid_states[idx] == 3:
                    return  # 点击无效，直接返回，路径不会添加，自然也不会画上去
                # =================================================

                if idx in self.r2_robot_path:
                    # 点击已在路径上的点：清除该点及其后的所有路径
                    idx_in_list = self.r2_robot_path.index(idx)
                    self.r2_robot_path = self.r2_robot_path[:idx_in_list]
                else:
                    # 点击未在路径上的点：如果是空路径，或相邻，则加入
                    if not self.r2_robot_path or self._is_adjacent(self.r2_robot_path[-1], idx):
                        self.r2_robot_path.append(idx)
                self._update_manual_text()

            elif self.manual_edit_type == "r1":
                if self.grid_states[idx] == 1:  # 必须是放置了R1方块的格子
                    if idx in self.r1_targets:
                        self.r1_targets.remove(idx)
                    else:
                        self.r1_targets.append(idx)
                self._update_manual_text()

            elif self.manual_edit_type == "r2":
                if self.grid_states[idx] == 2:  # 必须是放置了R2方块的格子
                    if idx in self.r2_targets:
                        self.r2_targets.remove(idx)
                    else:
                        self.r2_targets.append(idx)
                self._update_manual_text()

            # ========== 本次修改的核心 ==========
            self._validate_manual_r2_targets()  # 每次更新路径和方块都要重新检查
            # ==================================

            self.redraw_canvas()
            return  # 手动模式下不执行焦点切换逻辑
    ...
    def generate_overlay_images(self):
        ...
        self.overlay_images = {}
        # 字体大小随格子高度动态缩放
        font_size = max(10, int(self.cell_h * 0.25))
        try:
    ...
    def on_canvas_resize(self, event):
        ...
        # --- 新增：过滤因文字长度变化引起的微小布局抖动，防止无限循环重绘 ---
        if hasattr(self, 'canvas_width') and abs(self.canvas_width - event.width) <= 5 and abs(
                self.canvas_height - event.height) <= 5:
            return
    ...
    def redraw_canvas(self):
        ...
        font_size = max(24, int(self.cell_h * 0.15))
        text_map = {0: "0", 1: "1", 2: "2", 3: "F"}
    ...
    def draw_parsed_paths_graphics(self):
        ...
        # 2. 绘制 R2 夹取的方块：加入非法判定，不符合为红色，符合为黄色
        for idx in self.r2_targets:
            ...
            # 判断并分配颜色
            if getattr(self, 'is_manual_mode', False) and idx in getattr(self, 'invalid_r2_targets', []):
                box_color = "#ff0000"  # 不符合约束，红色
            else:
                box_color = "#ffff00"  # 符合约束，正常黄色

            self.canvas.create_rectangle(bx1, by1, bx2, by2, outline=box_color, width=box_line_w)
        ...
        if getattr(self, 'is_manual_mode', False):
            for idx in range(12):
                x1_f, y1_f = self.get_coords_from_index(idx)
                cx, cy = x1_f + self.cell_w / 2, y1_f + self.cell_h / 2

                # 动态自适应圆的半径大小
                radius = max(8, int(scale * 0.10))

                # 路径经过显红，未经过显白
                circle_color = "#ff0000" if idx in self.r2_robot_path else "#ffffff"

                self.canvas.create_oval(
                    cx - radius, cy - radius, cx + radius, cy + radius,
                    fill=circle_color, outline="#1e1e1e", width=2
                )
    ...
    def build_ros_command_block(self, parent, col):
        frame = tk.Frame(parent, bg='#2d2d2d', bd=2, relief=tk.SUNKEN, width=130)
        frame.grid(row=0, column=col, sticky='ns')
        frame.grid_propagate(False)  # 关闭尺寸传播，强制遵守width设定
        ttk.Label(frame, text="ROS 实时控制区", style='Header.TLabel', background='#2d2d2d').pack(pady=5)
        btn_container = tk.Frame(frame, bg='#2d2d2d')
        btn_container.pack(expand=True, fill=tk.BOTH, pady=3)
        # 发送路径按钮也同样修改
        send_path_btn = tk.Button(btn_container, text="发送路径", bg="#2b9348", fg="white", width=5,
                                  font=('DejaVu Sans', 12, 'bold'),
                                  command=self.execute_send_path)
        send_path_btn.pack(fill=tk.BOTH, expand=True)
        # 重启雷达指令按钮
        # --- 修改：定制化的重启雷达复合按钮区域（下方三按钮横向平分） ---
        lidar_frame = tk.Frame(btn_container, bg='#2d2d2d')
        lidar_frame.pack(fill=tk.BOTH, expand=True, pady=5)

        # 上方：重启执行大按钮
        restart_lidar_btn = tk.Button(lidar_frame, text="重启雷达", bg="#e85d04", fg="white", width=5,
                                      font=('DejaVu Sans', 12, 'bold'),
                                      command=self.execute_restart_lidar)
        restart_lidar_btn.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        ...
        # 子按钮：1
        self.btn_lidar_mode1 = tk.Button(mode_frame, text="1", font=('DejaVu Sans', 7, 'bold'),
                                         command=lambda: self.set_lidar_mode(1))
        # 修改点：添加 expand=True，使 1 和 2 也参与空间平分
        self.btn_lidar_mode1.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # 子按钮：2
        self.btn_lidar_mode2 = tk.Button(mode_frame, text="2", font=('DejaVu Sans', 7, 'bold'),
                                         command=lambda: self.set_lidar_mode(2))
        # 修改点：添加 expand=True
        self.btn_lidar_mode2.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        # 子按钮：both
        self.btn_lidar_both = tk.Button(mode_frame, text="all", font=('DejaVu Sans', 12, 'bold'), width=3,
        ...
        send_param_btn.pack(fill=tk.BOTH, expand=True)
        ...
        monitor_btn = tk.Button(
            ...
            bg="#F74DA9", fg="white",
            command=self.run_data_monitor_script
        )
        monitor_btn.pack(fill=tk.BOTH, expand=True, pady=5)
        # ============================================================
    ...
    def start_publish_loop(self):
        if not self.is_publishing:
            return
        msg = Int32()
        msg.data = getattr(self, "current_send_value", 0)
        self.current_pub.publish(msg)
        self.master.after(200, self.start_publish_loop)
    ...
    def process_next_command(self):
        ...
        elif cmd_type.startswith("check_on_"):
            task_key = cmd_type.replace("check_on_", "")
            self.current_pub = self.check_pub

            # 计算开指令的交替值
            vals = self.check_tasks_meta[task_key]["vals"]
            last_on = self.check_last_on_values[task_key]
            next_on_val = vals[1] if last_on == vals[0] else vals[0]

            self.current_send_value = next_on_val
            self.check_last_on_values[task_key] = next_on_val

            # 【状态更新】: 真正开始发送，状态由 5(排队) 或 0/4(空闲) 切换为 1(开发送中)
            self.check_states[task_key] = 1
            self.update_check_ui(task_key, "#BFB936", "发送自检\n指令中")
            self.is_publishing = True
            self.start_publish_loop()

        elif cmd_type.startswith("check_off_"):
            task_key = cmd_type.replace("check_off_", "")
            self.current_pub = self.global_check_off_pub

            # 计算关指令的交替值
            vals = self.check_tasks_meta[task_key]["vals"]
            last_off = self.check_last_off_values[task_key]
            next_off_val = vals[1] if last_off == vals[0] else vals[0]

            self.current_send_value = next_off_val
            self.check_last_off_values[task_key] = next_off_val

            # 【状态更新】: 真正开始发送，状态由 6(排队) 或 2(开启) 切换为 3(关发送中)
            self.check_states[task_key] = 3
            self.update_check_ui(task_key, "#BFB936", "发送关指\n令中")
            self.is_publishing = True
            self.start_publish_loop()
        # ==============================================
        else:
    ...
    def status_callback(self, msg):
        ...
        if msg.data == self.current_send_value:
            ...
            if self.current_command == 'send_path':
                ...
                # 修复5：区分手动与自动模式的成功表现，更新对应 UI
                if getattr(self, 'is_manual_mode', False):
                    self.update_ros_status("path", "路径指令：手动路径发送成功", "#50fa7b")
                else:
            ...
            elif self.current_command.startswith("check_on_"):
                task_key = self.current_command.replace("check_on_", "")
                if msg.data == self.current_send_value:
                    self.check_states[task_key] = 2
                    self.update_check_ui(task_key, "#0088ff", "设备\n自检中")  # 独立显示蓝色
                    self.current_command = None
                    self.is_publishing = False
                    self.master.after(500, self.process_next_command)

            elif self.current_command.startswith("check_off_"):
                task_key = self.current_command.replace("check_off_", "")
                if msg.data == self.current_send_value:
                    self.check_states[task_key] = 4
                    self.update_check_ui(task_key, "#299629", "设备自检\n完成")  # 独立显示绿色
                    self.current_command = None
                    self.is_publishing = False
                    self.master.after(500, self.process_next_command)
    ...
    def execute_send_path(self):
        ...
        if getattr(self, 'is_manual_mode', False):
            ...
            r1_invalid = any(self.grid_states.get(p) == 1 and p not in getattr(self, 'r1_targets', []) for p in
                             getattr(self, 'r2_robot_path', []))
            if r1_invalid:
                self.update_ros_status("path", "路径指令：路径上有R1未夹取，无法发送", "#ff0000")
                self.master.after(1000, lambda: self.update_ros_status("path", "手动路径：未发送", "#f1fa8c"))
                return
            ...
            r2_invalid = any(self.grid_states.get(p) == 2 and p not in getattr(self, 'r2_targets', []) for p in
                             getattr(self, 'r2_robot_path', []))
            if r2_invalid:
                self.update_ros_status("path", "路径指令：路径上有R2未夹取，无法发送", "#ff0000")
                self.master.after(1000, lambda: self.update_ros_status("path", "手动路径：未发送", "#f1fa8c"))
                return

            if not getattr(self, '_validate_manual_r2_targets', lambda: True)():
                # 修复1：增加静默拦截时的 UI 提示，避免完全无反应
                self.update_ros_status("path", "路径指令：R2位置不符合约束，无法发送", "#ff0000")
                self.master.after(1000, lambda: self.update_ros_status("path", "手动路径：未发送", "#f1fa8c"))
                return

            # 获取R2格式化好的文本（去除前缀）
            r2_list = self.r2_path_var.get().replace("R2：", "").strip()
            r2_list = r2_list.split("→")
            r2_text = ""
            for step in r2_list:
                r2_text += step + " "
            print(r2_text)
            # 格式化R1序列
            r1_text = " ".join([str(i + 1) for i in getattr(self, 'r1_targets', [])])
            final_command = f"{r2_text}0 {r1_text}".strip()

            # 修复2：写入下位机实际监听的 self.target_path，而非 auto_path.txt
            try:
                with open(self.target_path, "w", encoding="utf-8") as f:
                    f.write(final_command + "\n")
            except Exception as e:
                rospy.logerr(f"写入手动路径TXT失败: {e}")

            # 修复3：移除无效的 path_pub 检查，统一将其推入 ROS 任务队列进行处理
            self.enqueue_command('send_path')
            return  # 手动模式处理完毕后直接 return
    ...
    def _start_path_send(self):
        ...
        try:
            # 修复4：区分手动与自动模式，手动模式文件已写好，直接拉起发布循环
            if getattr(self, 'is_manual_mode', False):
                self.update_ros_status("path", f"路径指令：正在发送(手动)", "#f1fa8c")
            else:
                current_seq = self.get_current_merlin_seq()
            ...
            self.is_publishing = True
            self.start_publish_loop()
            rospy.loginfo(f"路径指令开始发送，值：{self.send_path_value}")
        except Exception as e:
    ...
    def _start_param_send(self):
        """参数发送核心逻辑：写txt +循环发布"""
        try:
            # === 修改：按json顺序取出数值，对特定键值加 100，空格分隔写入文件 ===
            vals = []
            for k in self.params_meta.keys():
                val = self.params_dict[k]
                if k in ["X", "Y"]:
                    val += 100
                vals.append(str(val))
    ...
    def build_check_block_content(self, parent):
        ...
        wrap_frame = tk.Frame(parent, bg="#2d2d2d")
        wrap_frame.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        ...
        # ========== 新增：yaw测试区块 ==========
        self.yaw_block_widget = self._create_yaw_test_widget(self.check_flow_frame)
        # ======================================
    ...
    def _create_single_check_widget(self, parent_container, task_key):
        ...
        title_lbl = tk.Label(block, text=meta["name"], bg="#383838", fg="white", font=("DejaVu Sans", 14, "bold"))
        title_lbl.pack(anchor="nw", padx=5, pady=3)
        ...
        btn_on = tk.Button(btn_frame, text="开", font=("DejaVu Sans", 20, "bold"), width=2, height=5,
                           command=lambda k=task_key: self.execute_check_cmd(k))
        btn_on.pack(side=tk.LEFT, padx=3)
        # === 新增：关按钮及绑定 ===
        btn_off = tk.Button(btn_frame, text="关", font=("DejaVu Sans", 20, "bold"), width=2, height=5,
                            command=lambda k=task_key: self.execute_check_off_cmd(k))
        btn_off.pack(side=tk.LEFT, padx=3)

        status_var = tk.StringVar(value="未执行")
        status_lbl = tk.Label(content_frame, textvariable=status_var, bg="#383838", fg="white",
                              font=("DejaVu Sans", 16, "bold"), justify=tk.RIGHT)
        status_lbl.pack(side=tk.RIGHT, padx=5)
    ...
    def _on_check_canvas_resize(self, event=None):
        # 支持手动调用：不传event时自动获取画布当前宽度
        if event is None:
            canvas_width = self.check_canvas.winfo_width()
        else:
            canvas_width = event.width
        ...
        cols = 2
        for i in range(cols):
            self.check_flow_frame.columnconfigure(i, weight=1, uniform="check_cols")
        new_block_width = max(150, (canvas_width // cols) - 20)

        for widget in self.check_block_widgets:
            widget.grid_forget()
        ...
        # 布局yaw测试区块
        if hasattr(self, 'yaw_block_widget'):
            # 先移除原有布局，避免重复grid导致的位置异常
            self.yaw_block_widget.grid_forget()
            last_row = (len(self.check_block_widgets) - 1) // cols + 1 if self.check_block_widgets else 0
            self.yaw_block_widget.config(width=canvas_width - 10)
            self.yaw_block_widget.grid(row=last_row, column=0, columnspan=cols, padx=0, pady=10, sticky="ew")
    ...
    def execute_check_cmd(self, task_key):
        """点击开按钮的逻辑拦截与排队状态显示"""
        state = self.check_states.get(task_key, 0)

        # 【强保护】: 如果正在发送(1,3)、已开启(2)、或已经在排队(5,6)，拒绝一切重复操作
        if state in [1, 2, 3, 5, 6]:
            return

        # 只有在状态为 0(空闲) 或 4(已关闭) 时，才允许入队
        # 检查队列主轴是否忙碌：若当前有指令在执行，则 UI 表现为排队
        if getattr(self, "current_command", None) is not None:
            self.check_states[task_key] = 5  # 状态 5：开排队中
            self.update_check_ui(task_key, "#BFB936", "发送开排\n队中")

        self.enqueue_command(f"check_on_{task_key}")

    def execute_check_off_cmd(self, task_key):
        """点击关按钮的逻辑拦截与排队状态显示"""
        state = self.check_states.get(task_key, 0)

        # 【强保护】: 如果未开启(0,4)、正在发送(1,3)、或已经在排队(5,6)，拒绝重复操作
        if state in [0, 1, 3, 4, 5, 6]:
            return

            # 只有在状态为 2(已开启/自检中) 时，才允许入队
        # 检查队列主轴是否忙碌：若当前有指令在执行，则 UI 表现为排队
        if getattr(self, "current_command", None) is not None:
            self.check_states[task_key] = 6  # 状态 6：关排队中
            self.update_check_ui(task_key, "#BFB936", "发送关排\n队中")

        self.enqueue_command(f"check_off_{task_key}")

    # =================== 新增yaw测试区块 ==================
    def _create_yaw_test_widget(self, parent_container):
        """创建铺满底部的 yaw 测试区块，支持 R1/R2 机器人切换，按钮紧贴标题右侧"""
        block = tk.Frame(parent_container, bg="#383838", bd=2, relief=tk.RAISED)

        # 标题栏：标题 + 紧贴的 R1/R2 切换按钮
        title_bar = tk.Frame(block, bg="#383838")
        title_bar.pack(fill=tk.X, padx=5, pady=3)

        # 标题文字
        tk.Label(title_bar, text="yaw测试", bg="#383838", fg="white",
                 font=("DejaVu Sans", 14, "bold")).pack(side=tk.LEFT)

        # R1/R2 切换按钮：紧贴标题右侧，仅保留极小间距
        self.yaw_robot_btns = {}
        # 按 R1、R2 顺序排列，紧贴标题
        for robot_id in [1, 2]:
            btn = tk.Button(
                title_bar, text=f"R{robot_id}", font=("DejaVu Sans", 12, "bold"),
                width=3, command=lambda rid=robot_id: self.switch_yaw_robot(rid)
            )
            # 第一个按钮（R1）与标题间距 8px，按钮之间间距 2px，实现紧贴效果
            pad_left = 8 if robot_id == 1 else 2
            btn.pack(side=tk.LEFT, padx=(pad_left, 0))
            self.yaw_robot_btns[robot_id] = btn

        # 初始化按钮高亮状态
        self._update_yaw_robot_btn_style()

        content_wrap = tk.Frame(block, bg="#383838")
        content_wrap.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

        # 数据结构初始化
        self.yaw_points = {1: None, 2: None}
        self.yaw_subs = {1: None, 2: None}
        self.yaw_record_buffer = {1: [], 2: []}
        self.yaw_recording = {1: False, 2: False}
        self.yaw_calculating = False

        self.yaw_ui_vars = {
            1: tk.StringVar(value="未记录"),
            2: tk.StringVar(value="未记录"),
            'yaw': tk.StringVar(value="未计算")
        }
        self.yaw_ui_labels = {}

        # 封装单行UI生成逻辑
        def create_row(parent, title, var_key, btn_text, cmd):
            row = tk.Frame(parent, bg="#383838")
            row.pack(fill=tk.X, pady=4)
            tk.Label(row, text=title, bg="#383838", fg="white", font=("DejaVu Sans", 14, "bold"), width=6,
                     anchor="e").pack(side=tk.LEFT)

            val_lbl = tk.Label(row, textvariable=self.yaw_ui_vars[var_key], bg="#383838", fg="white",
                               font=("DejaVu Sans", 14, "bold"), width=20, anchor="w")
            val_lbl.pack(side=tk.LEFT, padx=10)
            self.yaw_ui_labels[var_key] = val_lbl

            tk.Button(row, text=btn_text, font=("DejaVu Sans", 12, "bold"), width=8, command=cmd).pack(side=tk.LEFT,
                                                                                                       padx=10)

        create_row(content_wrap, "点一：", 1, "记录", lambda: self.start_record_yaw(1))
        create_row(content_wrap, "点二：", 2, "记录", lambda: self.start_record_yaw(2))
        create_row(content_wrap, "Yaw：", 'yaw', "计算", self.calculate_yaw)

        # 解决悬停在内部区域无法滚动的问题
        def _scroll(e):
            self.check_canvas.yview_scroll(-1 if e.num == 4 else 1, "units")

        for w in [block, content_wrap, title_bar] + content_wrap.winfo_children():
            w.bind("<Button-4>", _scroll)
            w.bind("<Button-5>", _scroll)
        ...
        # 订阅两个机器人的 odom 话题，回调中按当前选中机器人处理
        rospy.Subscriber(self.yaw_topic_map[1], Odometry, lambda msg: self._odom_yaw_cb(msg, 1))
        rospy.Subscriber(self.yaw_topic_map[2], Odometry, lambda msg: self._odom_yaw_cb(msg, 2))

        return block

    def start_record_yaw(self, pt_id):
        """触发记录操作，仅激活录制标志位"""
        # ================= 新增互斥拦截 =================
        if self.yaw_calculating:
            rospy.logwarn("系统正在计算中，请稍候再记录")
            return

        # 检查另一个点是否正在记录，如果是，则直接拦截本次操作
        other_pt_id = 2 if pt_id == 1 else 1
        if getattr(self, 'yaw_recording', {}).get(other_pt_id, False):
            rospy.logwarn(f"点 {other_pt_id} 正在记录中，请等待其完成再操作。")
            return
        # ==============================================

        self.yaw_ui_vars[pt_id].set("记录中...")
        self.yaw_ui_labels[pt_id].config(fg="#f1fa8c")  # 黄色
        self.yaw_record_buffer[pt_id] = []  # 清空历史采样缓存
        self.yaw_recording[pt_id] = True  # 开启录制开关

    def _odom_yaw_cb(self, msg, robot_id):
        """里程计回调，仅处理当前选中机器人的数据，对应通道标志位为 True 时采样"""
        # 非当前选中的机器人数据直接跳过
        if robot_id != self.current_yaw_robot:
            return

        for pt_id in [1, 2]:
            # 当前通道未开启录制则跳过
            if not self.yaw_recording.get(pt_id, False):
                continue

            buf = self.yaw_record_buffer[pt_id]
            if len(buf) < self.yaw_record_count:
                buf.append((msg.pose.pose.position.x, msg.pose.pose.position.y))

                if len(buf) == self.yaw_record_count:
                    self.yaw_recording[pt_id] = False  # 录满后关闭开关

                    # 计算点位平均值
                    avg_x = sum(p[0] for p in buf) / self.yaw_record_count
                    avg_y = sum(p[1] for p in buf) / self.yaw_record_count
                    self.yaw_points[pt_id] = (avg_x, avg_y)

                    # 绑定循环值避免异步错乱，主线程更新UI
                    def update_ui(pid=pt_id, ax=avg_x, ay=avg_y):
                        self.yaw_ui_vars[pid].set(f"{round(ax, 3):.3f}, {round(ay, 3):.3f}")
                        self.yaw_ui_labels[pid].config(fg="#50fa7b")  # 绿色
    ...
    def calculate_yaw(self):
        """计算两个点的 Yaw 角度"""
        # ================= 新增拦截 =================
        # 如果任何一个点正在记录中，直接拦截，不允许计算
        if self.yaw_recording.get(1, False) or self.yaw_recording.get(2, False):
            rospy.logwarn("点位记录中，禁止计算yaw")
            return
        # ==========================================

        self.yaw_calculating = True

        p1 = self.yaw_points[1]
        p2 = self.yaw_points[2]
        ...
        # math.atan2 是 arctan 的安全变体，能够自动处理 dx 为 0 的状况并区分四个象限
        yaw_rad = math.atan2(dy, dx)
        # yaw_deg = math.degrees(yaw_rad)

        self.yaw_ui_vars['yaw'].set(f"{round(yaw_rad, 5):.5f} rad")
        self.yaw_ui_labels['yaw'].config(fg="#00ff00")

        self.yaw_calculating = False

    def switch_yaw_robot(self, robot_id):
        """切换yaw测试对应的机器人，切换时清空所有记录与计算结果"""
        if robot_id == self.current_yaw_robot:
            return

        self.current_yaw_robot = robot_id
        ...
        # 重置UI显示
        self.yaw_ui_vars[1].set("未记录")
        self.yaw_ui_labels[1].config(fg="white")
        self.yaw_ui_vars[2].set("未记录")
        self.yaw_ui_labels[2].config(fg="white")
        self.yaw_ui_vars['yaw'].set("未计算")
        self.yaw_ui_labels['yaw'].config(fg="white")

        # 更新按钮高亮
        self._update_yaw_robot_btn_style()

        rospy.loginfo(f"yaw测试已切换至 R{robot_id}，话题：{self.yaw_topic_map[robot_id]}")

    def _update_yaw_robot_btn_style(self):
        """更新 R1/R2 切换按钮的选中高亮样式"""
        for rid, btn in self.yaw_robot_btns.items():
            if rid == self.current_yaw_robot:
                btn.config(bg="#f1fa8c", fg="black")
            else:
                btn.config(bg="#44475a", fg="white")
    ...
    def build_monitor_block_content(self, parent):
        ...
        precision_frame.pack(fill=tk.X, padx=5)

        # 靠左写文字“精度：”
        tk.Label(precision_frame, text="精度：", bg="#2d2d2d", fg="white", font=("DejaVu Sans", 16, "bold")).pack(
            side=tk.LEFT)
        ...
        for p in precisions:
            btn = tk.Button(
                precision_frame, text=f".{p}", font=("DejaVu Sans", 18, "bold"),
        ...
        # 最后放一个字符 f
        tk.Label(precision_frame, text="f", bg="#2d2d2d", fg="white", font=("DejaVu Sans", 16, "bold")).pack(
            side=tk.LEFT, padx=1)
        ...
        wrap_frame = tk.Frame(parent, bg="#2d2d2d")
        wrap_frame.pack(fill=tk.BOTH, expand=True, padx=5)
    ...
    def _create_single_monitor_widget(self, parent_container, item):
        ...
        block = tk.Frame(parent_container, bg="#383838", bd=2, relief=tk.RAISED, height=130)
        block.pack_propagate(False)

        tk.Label(block, text=item["name"], bg="#383838", fg="white", font=("DejaVu Sans", 14, "bold")).pack(anchor="w",
                                                                                                            padx=5,
                                                                                                            pady=3)
        ...
        data_label = tk.Label(
            ...
            font=("DejaVu Sans", 12, "bold"), justify=tk.LEFT, anchor="nw"
        )
        data_label.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def build_script_block_content(self, parent):
        ...
        self.control_bar.pack(side=tk.TOP, fill=tk.X, padx=10, pady=2)
        ...
        btn_run_term = tk.Button(inner_bar, text="终端运行", bg="#44475a", fg="white",
                                 font=('DejaVu Sans', 16, 'bold'), width=12,
                                 command=self.run_selected_script_in_terminal)
        btn_run_term.pack(side=tk.LEFT, padx=3, pady=2)
        # 修改：将父容器从 self.control_bar 改为 inner_bar
        btn_stop = tk.Button(inner_bar, text="停止", bg="#44475a", fg="white",
                             font=('DejaVu Sans', 16, 'bold'), width=12, command=self.stop_selected_script)
        btn_stop.pack(side=tk.LEFT, padx=3, pady=2)
        # 2. 滚动的脚本列表区域
        list_container = tk.Frame(frame, bg='#2d2d2d')
        list_container.pack(expand=True, fill=tk.BOTH, padx=10, pady=2)
        ...
        status_label = tk.Label(frame, textvariable=self.script_status_var,
                                bg='#1e1e1e', fg='#50fa7b', font=('DejaVu Sans Mono', 10), height=2)
