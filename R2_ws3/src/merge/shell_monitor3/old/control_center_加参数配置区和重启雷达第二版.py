...
class TouchscreenUI:
    def __init__(self, master):
        ...
        self.ros_status_vars = {
            "path": tk.StringVar(value="路径指令：未发送任何路径"),
        ...
        # 参数ROS发布器，5/6交替
        self.send_param_pub = rospy.Publisher('/scripts/send_param', Int32, queue_size=10)
    ...
    def build_ros_status_bar(self, parent):
        ...
        for key, var in self.ros_status_vars.items():
            lbl = tk.Label(inner_frame, textvariable=var, bg="#2d2d2d",
                           fg="#f1fa8c", font=('DejaVu Sans', 13, 'bold'))
        ...
        try:
            ...
            for k, v in raw_cfg.items():
                ...
                self.params_meta[k] = {
                    "name": name,
                    "range": (minv, maxv),
                    "map": map_dict  # 新增：存储映射字典
                }
                self.params_dict[k] = minv
    ...
    def build_left_panel(self, parent, col):
        ...
        # 新增参数配置Tab按钮
        self.btn_param_view = tk.Button(title_bar, text="参数配置区", font=('DejaVu Sans', 18, 'bold'),
    ...
    def switch_left_view(self, view_name):
        ...
        reset_btn_style(self.btn_script_view)
    ...
    def build_param_block_content(self, parent):
        ...
        # 左右1:1均分
        wrap_frame.columnconfigure(0, weight=1, uniform="param_col")
        wrap_frame.columnconfigure(1, weight=1, uniform="param_col")
        wrap_frame.rowconfigure(0, weight=1)

        # 封装生成单栏滚动容器函数
        def create_scroll_column(parent_col):
            scroll_wrap = tk.Frame(parent_col, bg="#2d2d2d")
            scroll_wrap.pack(fill=tk.BOTH, expand=True, padx=5)
            bar = tk.Scrollbar(scroll_wrap, orient=tk.VERTICAL)
            bar.pack(side=tk.RIGHT, fill=tk.Y)
            canvas = tk.Canvas(scroll_wrap, bg="#2d2d2d", highlightthickness=0, yscrollcommand=bar.set)
        ...
        # 左右两栏容器
        self.param_left_col = create_scroll_column(wrap_frame)
        self.param_right_col = create_scroll_column(wrap_frame)
        self.param_left_col.grid(row=0, column=0, sticky="nsew")
        self.param_right_col.grid(row=0, column=1, sticky="nsew")
        # 参数均分左右两栏
        param_keys = list(self.params_meta.keys())
        mid = len(param_keys) // 2
        left_keys = param_keys[:mid]
        right_keys = param_keys[mid:]
        # 生成左侧参数块
        for key in left_keys:
            self._create_single_param_widget(self.param_left_col, key)
        # 生成右侧参数块
        for key in right_keys:
            self._create_single_param_widget(self.param_right_col, key)

    def _create_single_param_widget(self, parent_container, param_key):
        """单个参数UI块，横向铺满父容器宽度（放大版）"""
        meta = self.params_meta[param_key]
        name = meta["name"]
        minv, maxv = meta["range"]
        map_dict = meta["map"]  # 获取当前参数的映射字典

        # 放大块的上下间距
        block = tk.Frame(parent_container, bg="#383838", bd=2, relief=tk.RAISED)
        block.pack(fill=tk.X, padx=4, pady=10)
        # 放大名称字号与内边距
        tk.Label(block, text=name, bg="#383838", fg="white", font=("DejaVu Sans", 28, "bold")).pack(anchor="w", padx=10,
                                                                                                    pady=6)
        # 放大按钮行的上下间距
        ...
        for val in range(minv, maxv + 1):
            # 核心修改：优先使用map映射，无映射则显示原数字
            txt = map_dict.get(val, str(val))

            # 放大按钮字号与内边距，增大触屏点击区域
            btn = tk.Button(btn_wrap, text=txt, font=("DejaVu Sans", 20, "bold"),
    ...
    def update_path_status_display(self):
        ...
        if not self.current_map_sent_plan:
            self.update_ros_status("path", "路径指令：未发送任何路径", "#f1fa8c")
        else:
            sent_r1, sent_r2 = self.current_map_sent_plan
            base_text = f"路径指令：已发送 R1={sent_r1} R2={sent_r2}"
    ...
    def status_callback(self, msg):
        ...
        if msg.data == self.current_send_value:
            ...
            if self.current_command == 'send_path':
                ...
                rospy.loginfo(f"重启雷达指令下一次发送值：{self.restart_lidar_value}")
                self.update_ros_status("lidar", "雷达重启：执行成功", "#50fa7b")
    ...
    def _start_path_send(self):
        ...
        try:
            ...
            # 更新状态栏
            self.update_ros_status("path", f"路径指令：正在发送 (值={self.send_path_value})...", "#f1fa8c")
            # 设置当前发送参数
    ...
    def execute_restart_lidar(self):
        ...
        if click_count >= 3:
            ...
            self.update_ros_status("lidar", f"雷达重启：再点{remain}次确认 (2s内)", "#f1fa8c")
    ...
    def _start_lidar_restart(self):
        ...
        try:
            ...
            self.update_ros_status("lidar", f"雷达重启：正在发送 (值={self.restart_lidar_value})...", "#f1fa8c")
            # 设置当前发送参数
    ...
    def _start_param_send(self):
        ...
        try:
            ...
            # 更新状态栏
            self.update_ros_status("param", f"参数指令：正在发送 (值={self.send_param_value})...", "#f1fa8c")
            # 配置发布器与发送值
