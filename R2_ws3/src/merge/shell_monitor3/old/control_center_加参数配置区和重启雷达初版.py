...
class TouchscreenUI:
    ...
    def _load_params_json(self):
        ...
        try:
            ...
            for k, v in raw_cfg.items():
                ...
                self.params_meta[k] = {"name":name, "range":(minv, maxv)}
    ...
    def _create_single_param_widget(self, parent_container, param_key):
        ...
        block.pack(fill=tk.X, padx=4, pady=6)
        # 中文名称行
        tk.Label(block, text=name, bg="#383838", fg="white", font=("DejaVu Sans", 19, "bold")).pack(anchor="w", padx=8,
                                                                                                    pady=4)
        # 数值按钮行
        btn_wrap = tk.Frame(block, bg="#383838")
        btn_wrap.pack(anchor="w", padx=8, pady=4)
        self.param_blocks[param_key] = {}
        for val in range(minv, maxv + 1):
            # 0-1映射True/False
            if (minv, maxv) == (0, 1):
                txt = "True" if val == 1 else "False"
            else:
                txt = str(val)
            btn = tk.Button(btn_wrap, text=txt, font=("DejaVu Sans", 16, "bold"),
    ...
    def status_callback(self, msg):
        ...
        if msg.data == self.current_send_value:
            ...
            if self.current_command == 'send_path':
                ...
                self.send_test_value = 6 if self.send_test_value == 5 else 5
                rospy.loginfo(f"重启雷达指令下一次发送值：{self.send_test_value}")
