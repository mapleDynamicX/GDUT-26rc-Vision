...
class TouchscreenUI:
    def __init__(self, master):
        ...
        # 参数ROS发布器，5/6交替
        self.send_param_pub = rospy.Publisher('/scripts/send_param', Int32, queue_size=3)
        ...
        # ========== 路径指令（原有逻辑整合） ==========
        self.send_path_value = 1
        self.send_path_pub = rospy.Publisher('/scripts/send_path', Int32, queue_size=3)
        # ========== 重启雷达指令 ==========
        self.restart_lidar_value = 3  # 用3/4交替，与路径的1/2区分，避免应答冲突
        self.restart_lidar_value2 = 3
        self.lidar_restart_pub = rospy.Publisher('/scripts/restart_lidar', Int32, queue_size=3)
        self.lidar_restart_pub2 = rospy.Publisher('/scripts/restart_lidar2', Int32, queue_size=3)
        # 新增：重启雷达防误触 - 时间戳队列
    ...
    def build_param_block_content(self, parent):
        ...
        self.param_canvas = tk.Canvas(wrap_frame, bg="#2d2d2d", highlightthickness=0,
                                      yscrollcommand=scrollbar.set, yscrollincrement=1)
        ...
        # ========== 新增：将整个参数内部容器及所有方块、文字、按钮绑定触屏拖拽 ==========
        self._bind_touch_scroll(self.param_canvas, self.param_canvas)
    ...
    def _on_touch_scroll_start(self, event):
        """记录触屏拖拽时的初始全局 Y 坐标 (y_root 可防止跨组件拖拽时坐标跳变)"""
        self._drag_start_y = event.y_root

    def _on_touch_scroll_move(self, event, canvas):
        """计算拖拽距离并滚动指定的 Canvas"""
        if not hasattr(self, '_drag_start_y'):
            return
        delta_y = self._drag_start_y - event.y_root

        # 配合 Canvas 的 yscrollincrement=1，实现 1:1 的像素级像素跟随滑动
        canvas.yview_scroll(delta_y, "units")
        self._drag_start_y = event.y_root

    def _bind_touch_scroll(self, widget, canvas):
        """递归绑定拖拽事件，使得按住背景或任何子组件(如标签、方块)都能触发滑动"""
        # 排除原生滚动条，防止与自带拖拽逻辑冲突
        if not isinstance(widget, tk.Scrollbar):
            widget.bind("<ButtonPress-1>", self._on_touch_scroll_start, add="+")
            widget.bind("<B1-Motion>", lambda e: self._on_touch_scroll_move(e, canvas), add="+")

        for child in widget.winfo_children():
            self._bind_touch_scroll(child, canvas)
    ...
    def status_callback(self, msg):
        ...
        if msg.data == self.current_send_value:
            ...
            elif self.current_command == 'restart_lidar2':
                self.restart_lidar_value2 = 4 if self.restart_lidar_value2 == 3 else 3
                rospy.loginfo(f"重启雷达指令2下一次发送值：{self.restart_lidar_value2}")
                self.update_ros_status("lidar", "雷达重启2：执行成功", "#50fa7b")
    ...
    def _start_lidar_restart(self):
        ...
        try:
            ...
            self.current_pub = self.lidar_restart_pub2
