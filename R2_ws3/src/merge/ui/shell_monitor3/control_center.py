...
class TouchscreenUI:
    ...
    def _build_plan_table(self):
        ...
        tk.Label(self.table_frame, text="R1\\R2", fg="black", bg=bg_transparent, font=('DejaVu Sans', 13, 'bold'),
        ...
        for col_idx, r2_cnt in enumerate(r2_range, start=1):
            tk.Label(self.table_frame, text=f"{r2_cnt}个", fg="black", bg=bg_transparent,
                     font=('DejaVu Sans', 12, 'bold')).grid(row=0, column=col_idx, sticky='nsew', padx=1, pady=1)

        for row_idx, r1_cnt in enumerate(r1_range, start=1):
            tk.Label(self.table_frame, text=f"{r1_cnt}个", fg="black", bg=bg_transparent,
                     font=('DejaVu Sans', 12, 'bold')).grid(row=row_idx, column=0, sticky='nsew', padx=1, pady=1)
            for col_idx, r2_cnt in enumerate(r2_range, start=1):
    ...
    def _update_match_mode_state(self):
        ...
        if self.is_challenge_mode:
            ...
            self.match_mode_toggle_btn.config(text="切换挑战赛")
            self.databases = self.regular_databases
    ...
    def _create_single_param_widget(self, parent_container, param_key):
        ...
        btn_wrap.pack(anchor="w", padx=5, pady=4)
    ...
    def build_grid_block_content(self, parent):
        ...
        self.manual_mode_btn = tk.Button(btn_top_frame, text="切换手动路径", bg="#44475a", fg="white",
                                         font=('DejaVu Sans', 17, 'bold'), height=2, command=self.toggle_manual_mode)
        self.manual_mode_btn.pack(side=tk.LEFT)

        # 修改：将原本宽度 30 缩小，给新按钮腾位置
        self.court_toggle_btn = tk.Button(btn_top_frame, text="切换为蓝场", font=('DejaVu Sans', 17, 'bold'),
                                          width=9, height=2, command=self.toggle_court_side)
        self.court_toggle_btn.pack(side=tk.LEFT, padx=2)

        # 新增：赛制切换按钮
        self.match_mode_toggle_btn = tk.Button(btn_top_frame, text="切换挑战赛", font=('DejaVu Sans', 17, 'bold'),
                                               width=9, height=2, command=self.toggle_match_mode)
        ...
        self.r2_path_label = tk.Label(info_container, textvariable=self.r2_path_var, fg="#000000", bg="#2d2d2d",
                 font=('DejaVu Sans', 15, 'bold'), anchor=tk.W, width=35, justify=tk.LEFT, wraplength=450)
        ...
        self.r1_path_label = tk.Label(info_container, textvariable=self.r1_path_var, fg="#0021FA", bg="#2d2d2d",
                 font=('DejaVu Sans', 15, 'bold'), anchor=tk.W, width=35, justify=tk.LEFT, wraplength=1200)
