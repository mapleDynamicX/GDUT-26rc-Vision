...
namespace Ten
{
    ...
    void lidarrestartCallbackR2(const std_msgs::Int32::ConstPtr& msg)
    {
        ...
        else
        {
            if(_is_tf_init_.load())
            {
                std::cout << "_is_tf_init_.load()" << std::endl;
                responseR2(last);
            }
            //responseR2(last);
        }
    ...
    void parameterCallbackR2(const std_msgs::Int32::ConstPtr& msg)
    {
        ...
        if(receive != last)
        {
            ...
            if(length)
            {
                ...
                double yyy = 0;
                //x
                if(Ten::_input_parameter_[8] != 0)
                {
                    xxx = ((double)Ten::_input_parameter_[8] - 100.0) / 1000.0;
                }
                //y
                if(Ten::_input_parameter_[9] != 0)
                {
                    yyy = ((double)Ten::_input_parameter_[9] - 100.0) / 1000.0;
                }

                //2号启动区
                if(Ten::_input_parameter_[5] == 0)
                {
                    //红
                    if(Ten::_input_parameter_[0] == 0)
                    {
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_x_.store(_r2_red_0_dx_);
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_y_ .store(_r2_red_0_dy_);
                        Ten::superstratum::_r2_xyzrpy_init_error_rpy_yaw_ .store(_r2_red_0_dyaw_);
                    }
                    //蓝
                    else if(Ten::_input_parameter_[0] == 1)
                    {
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_x_.store(_r2_blue_0_dx_);
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_y_ .store(_r2_blue_0_dy_);
                        Ten::superstratum::_r2_xyzrpy_init_error_rpy_yaw_ .store(_r2_blue_0_dyaw_);
                    }
                }
                //原点
                else if(Ten::_input_parameter_[5] == 1)
                {
                    //红
                    if(Ten::_input_parameter_[0] == 0)
                    {
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_x_.store(_r2_red_1_dx_ - xxx);
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_y_ .store(_r2_red_1_dy_ - yyy);
                        Ten::superstratum::_r2_xyzrpy_init_error_rpy_yaw_ .store(_r2_red_1_dyaw_);
                    }
                    //蓝
                    else if(Ten::_input_parameter_[0] == 1)
                    {
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_x_.store(_r2_blue_1_dx_ - xxx);
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_y_ .store(_r2_blue_1_dy_ + yyy);
                        Ten::superstratum::_r2_xyzrpy_init_error_rpy_yaw_ .store(_r2_blue_1_dyaw_);
                    }
                }
                //3区
                else if(Ten::_input_parameter_[5] == 2)
                {
                    //红
                    if(Ten::_input_parameter_[0] == 0)
                    {
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_x_.store(_r2_red_2_dx_);
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_y_ .store(_r2_red_2_dy_);
                        Ten::superstratum::_r2_xyzrpy_init_error_rpy_yaw_ .store(_r2_red_2_dyaw_);
                    }
                    //蓝
                    else if(Ten::_input_parameter_[0] == 1)
                    {
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_x_.store(_r2_blue_2_dx_);
                        Ten::superstratum::_r2_xyzrpy_init_error_xyz_y_ .store(_r2_blue_2_dy_);
                        Ten::superstratum::_r2_xyzrpy_init_error_rpy_yaw_ .store(_r2_blue_2_dyaw_);
                    }
                }
                ...
                while(Ten::_PARAMETER_FLAG_.read_flag() && Ten::_TREADPOOL_FLAG_.read_flag())
                {
                    ...
                    serial.serial_send(MFparameter, 9, length - 2);
                    usleep(100*1000);
                }
                responseR2(last);
            }
    ...
    void tf_monitorR2(nav_msgs::Odometry msg)
    {
        ...
        pose_fastlio._xyz._x -= _stead_tf_dx_.load();
        pose_fastlio._xyz._y -= _stead_tf_dy_.load();
        ...
        if(Ten::_POINT_LIO_RUN_FLAG_.read_flag())
        {
            ...
            int times = 0;
            while(Ten::_TREADPOOL_FLAG_.read_flag() && times < 200)
            {
        ...
        else
        {
            ...
            ros::Rate sl(Ten::_laser_pub_hz_*2);
            int times = 0;
            while(Ten::_TREADPOOL_FLAG_.read_flag() && times < 3)
            {
                nav_msgs::Odometry odo;
                if(!Ten::_TF_GET_.get_latest(odo))
                {
            ...
            Ten::XYZRPY xyzrpy_errorx;
            xyzrpy_errorx._rpy._yaw = Ten::superstratum::_r2_xyzrpy_init_error_rpy_yaw_.load();
            result = Ten::math::transform_matrixtoXYZRPY_fixed(Ten::math::XYZRPYtotransform_matrix_fixed(xyzrpy_errorx)*Ten::math::XYZRPYtotransform_matrix_fixed(result));
            debug.pose = result;
    ...
    void odomCallbackR2(const nav_msgs::Odometry::ConstPtr& msg)
    {
        // _stead_tf_dx_.store(0.0);
        // _stead_tf_dy_.store(0.0);
        static int _init_frame_num_ = 20;
        static int _error_frame_num_ = 20;
        static double _dx_ = 0.0;
        static double _dy_ = 0.0;

        if(Ten::_is_tf_init_.load() == false)
        {
            Ten::XYZRPY pose_fastlio = Ten::Nav_Odometrytoxyzrpy(*msg);
            double distance = pose_fastlio.Eclidean_distance(Ten::XYZRPY());
            if(distance > 0.09)
            {
            ...
            {
                _dx_ += pose_fastlio._xyz._x;
                _dy_ += pose_fastlio._xyz._y;
                _init_frame_num_--;
            }

            if(_init_frame_num_ <= 0)
            {
                _stead_tf_dx_.store(_dx_ / 20.0);
                _stead_tf_dy_.store(_dy_ / 20.0);
                Ten::_is_tf_init_.store(true);
            }
            if(_error_frame_num_ <= 0)
            {
                _stead_tf_dx_.store(0.0);
                _stead_tf_dy_.store(0.0);
                Ten::_is_tf_init_.store(true);
