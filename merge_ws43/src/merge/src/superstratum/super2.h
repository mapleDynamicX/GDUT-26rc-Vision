...
namespace Ten
{
    namespace superstratum
    {
        ...
        struct super_init{
            ...
            std::string model_path = "/home/robocon/rc2026/model/yolo11s_roi12_atten_17/yolo11s_roi12_atten_17";
            ...
            super_init() : yolo_han2_(model_path,device)
                          ,yolo_detector_("/home/robocon/rc2026/model/juanZhou_cls1/best", "cpu", mapping_)        // 用【初始化列表】调用 yolo_han2_ 的构造函数
            {
            ...
            */
            ...
            )
            {
                ...
                for (int i = 0; i < classifier_.size(); i++)
                {
                    ...
                    if(classifier_[i] == 4 && std::abs(confidence_[i] - 1) < 0.02 && place[i] == 0
                        && now_count[ classifier_[i] - 1] < limit_count[ classifier_[i] - 1])        // 极高置信度且 为空类 直接确定结果
                    {
                        final_result[i] = classifier_[i];
                        now_count[ final_result[i] - 1] += 1;
                        is_process[i] = 1;
