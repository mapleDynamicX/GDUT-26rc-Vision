...
class TouchscreenUI:
    def __init__(self, master):
        ...
        # 状态机: 0(空闲), 1(开指令发送中), 2(设备自检中/开完成), 3(关指令发送中), 4(自检完成/关完成), 5(开排队中), 6(关排队中)
        self.check_states = {k: 0 for k in self.check_tasks_meta.keys()}

        # 记录下一次发送的值
        self.check_send_values = {k: 1 for k in self.check_tasks_meta.keys()}  # 各设备独立的开信号交替
        self.shared_check_off_value = 1  # 全局唯一的关信号交替（1、2交替）
        ...
        # 所有设备完全共用这一个关话题和发布器
        self.shared_off_topic = "/scripts/global_check_off"
        self.shared_check_off_pub = rospy.Publisher(self.shared_off_topic, Int32, queue_size=1)
        # ===================================================================================
    ...
    def process_next_command(self):
        ...
        if cmd_type == 'send_path':
            ...
            task_key = cmd_type.replace("check_", "")

            self.current_pub = self.check_pubs[task_key]
            self.current_send_value = self.check_send_values[task_key]
            ...
            # === 修改：消费全局唯一的 check_all_off 指令 ===
            # === 修改：消费全局唯一的 global_check_off 指令 ===
        elif cmd_type == "global_check_off":
            self.current_pub = self.shared_check_off_pub
            self.current_send_value = self.shared_check_off_value

            # 将所有处于 自检中(状态2) 或 排队中(状态6) 的块，一键同步切换为“发送关指令中”，转黄色
            for k, v in self.check_states.items():
                if v in [2, 6]:
                    self.check_states[k] = 3  # 3: 关指令发送中
                    self.update_check_ui(k, "#BFB936", "发送关指\n令中")
    ...
    def status_callback(self, msg):
        ...
        if msg.data == self.current_send_value:
            ...
            if self.current_command == 'send_path':
                ...
                task_key = self.current_command.replace("check_", "")

                # 翻转下次发送的值：1 变 2，2 变 1
                self.check_send_values[task_key] = 2 if self.check_send_values[task_key] == 1 else 1

                self.check_states[task_key] = 2
                # --- 修改：设备开完并收到应答，显示蓝色自检中 ---
                self.update_check_ui(task_key, "#0088ff", "设备\n自检中")  # 蓝色
                # ------------------------------------------------

                # === 新增：处理关指令的应答 ===
                # === 修改：处理全局关指令的应答 ===
            elif self.current_command == "global_check_off":
                # 全局关话题独立的翻转：1 变 2，2 变 1
                self.shared_check_off_value = 2 if self.shared_check_off_value == 1 else 1

                # 将所有处于 关指令发送中(状态3) 的块，统一转为“设备自检完成”，转绿色
                for k, v in self.check_states.items():
                    if v == 3:
                        self.check_states[k] = 4  # 4: 自检完成
                        self.update_check_ui(k, "#299629", "设备自检\n完成")  # 绿色
            # ==================================
            # 复位当前指令，调度下一条
            self.current_command = None
            self.update_ros_status("common", "系统状态：空闲", "#50fa7b")
    ...
    def execute_check_cmd(self, task_key):
        ...
        if state in [1, 5]: return  # 仅拦截发送中和排队中的重复点击

        # === 如果当前 ROS 正在发送/等待其他指令，先显示等待发送 ===
        if self.is_publishing:
            # 状态机：5 代表处于排队等待状态
            self.check_states[task_key] = 5
            self.update_check_ui(task_key, "#BFB936", "等待\n发送中")
        # =========================================================

        self.enqueue_command(f"check_{task_key}")

        # === 新增：点击发送关的逻辑拦截器 ===

    def execute_check_off_cmd(self, task_key):
        """点击任何自检块的‘关’按钮逻辑拦截器"""
        # 1. 拦截：如果全局关指令已经在队列中或正在执行，直接静默拦截
        if self.current_command == "global_check_off" or "global_check_off" in self.command_queue:
            return

        # 2. 拦截：获取当前所有处于“设备自检中(状态2)”的设备
        active_devices = [k for k, v in self.check_states.items() if v == 2]
        if not active_devices:
            return  # 如果没有任何设备处于自检中，按关无反应

        # 3. 如果当前 ROS 正在发送其他指令，先让所有开着的块同步转为“等待发送中”
        if self.is_publishing:
            for k in active_devices:
                self.check_states[k] = 6  # 6: 关指令排队中
                self.update_check_ui(k, "#BFB936", "等待\n发送中")

        # 【修改】触发全局唯一的关指令入队
        self.enqueue_command("global_check_off")
    # ====================================
