...
uint64 timebase           # 首点时间基准
uint32 point_num          # 点云总数
uint8  lidar_id           # 激光雷达设备ID
uint8[3]  rsvd            # 保留字段
...
| 字段名          | 类型      | 单位    | 说明                |
| ------------ | ------- | ----- | ----------------- |
| offset_time  | uint32  | ns    | 相对 timebase 的时间偏移 |
| x            | float32 | m     | X 坐标（前向）          |
| y            | float32 | m     | Y 坐标（左向）          |
| z            | float32 | m     | Z 坐标（上向）          |
| reflectivity | uint8   | 0~255 | 反射强度              |
| tag          | uint8   | 0~255 | 回波信息与保留位          |
| line         | uint8   | 0~255 | 激光线编号             |
