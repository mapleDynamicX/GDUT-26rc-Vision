...
// std::once_flag Ten_serial::serial_flag_;
    ...
    /**
        ...
        @param wait_timeout_ms: 单步等待超时(ms)，默认100ms
        @return bool 解析成功返回true，超时/异常返回false
    */
    bool Ten_serial::serial_read2(void* p, uint8_t& received_frame_id, uint8_t& received_length, uint32_t wait_timeout_ms) 
    {
        std::lock_guard<std::mutex> lock(read_mtx_);
        if(this == nullptr || p == nullptr)
        {
        ...
        int num_count = wait_timeout_ms*10;
