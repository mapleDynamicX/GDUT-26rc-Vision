...
struct segmentation
{
    ...
    segmentation(float x_min, float y_min, float z_min, float x_maz, float y_max, float z_max)
    {
        _x_min = x_min;
        _y_min = y_min;
        _z_min = z_min;
...
public:
    ...
    _cloud_boxs(new pcl::PointCloud<pcl::PointXYZI>)
    {
        ...
        _have_box_pub = _nh.advertise<rc_msgs::serial>("/serial/box_is_have", 2);
        ...
        for(int i = 0; i < 4; i++)
        {
            for(int j = 0; j < 3; j++)
            {
                segmentation s(2.9 + i*_dx_, 3.3 - (j+1)*_dy_, -0.67, 2.9 + (i+1)*_dx_, 3.3 - j*_dy_, -0.3);
                _seg.push_back(s);
            }
        }
...
void registering()
{
    ...
    *_cloud_boxs = *cloud_filtered;
    std::cout<< "降采样2######" << _cloud_boxs->points.size()<<std::endl;
    if(_cloud_boxs->points.size() > 1000)
    {
        pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_filtered2(new pcl::PointCloud<pcl::PointXYZI>);
        ...
        *cloud_filtered2 = *_cloud_boxs;
        _cloud_map.push_back(cloud_filtered2);
        // if(_cloud_boxs->points.size() > 20000)
        // _cloud_boxs->resize(20000);
        _cloud_boxs->points.clear();//size == 0
        //_cloud_boxs->resize(100);
    }
...
void pubcloud()
{
    ...
    {
        int tmp = 0;
        for (auto &p : cloud_map->points)
        {
            if (p.x >= _seg[i]._x_min && p.x <= _seg[i]._x_max &&
                p.y >= _seg[i]._y_min && p.y <= _seg[i]._y_max &&
                p.z >= _seg[i]._z_min && p.z <= _seg[i]._z_max) 
            {
                //_cloud_box->points.push_back(p);
                tmp++;
                if(tmp >= 500)
                {
                    serial_msg.map.push_back(1);
                    break;
                }
            }
        }
    ...
    if(_cloud_map.size() >= 7)
    {
        _have_box_pub.publish(serial_msg);
    }
