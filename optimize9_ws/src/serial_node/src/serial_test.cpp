...
namespace mcu0_serial
{
    ...
    size_t serial_mcu::serial_send2(void* p, uint8_t frame_id, uint8_t length)
    {
        // 构建数据帧：帧头+ID+数据长度+数据+CRC+帧尾
        ...
        //异或校验
        buff_msg[4 + length] = calculateArrayParity(&buff_msg[4], length);
    ...
    bool serial_mcu::serial_read(uint8_t* received_frame_id, float msgs[], uint8_t* received_length) {
            ...
            */
            ...
            if (byte == FRAME_HEAD_0) {
                ...
                *received_frame_id = framein_.frame_id;
                ...
                //std::cout<<"data_length"<<expected_data_length<<std::endl;
        ...
        // 串口数据接收函数
    bool serial_mcu::serial_read2(void* p, uint8_t& received_frame_id, uint8_t& received_length) {
        if (!serial_.isOpen()) {
            ROS_ERROR("Serial port is not open.");
            return false;
        }
        ...
        while (serial_.available() > 0) {
            ...
            */
            ...
            if (byte == FRAME_HEAD_0) {
                ...
                if (serial_.read(&data_length, 1) != 1) 
                {
                    std::cout<<"(int)data_length"<<(int)data_length<<std::endl;
                    continue;
                }
                
                size_t expected_data_length = data_length;
                //std::cout<<"expected_data_length"<<expected_data_length<<std::endl;
                // Validate data length检查数据长度是否超出缓冲区容量
                if (expected_data_length > sizeof(framein_.data.buff_msg)) {
                    ROS_WARN("Data too long: %d", expected_data_length);
                    continue;
                ...
                received_length = expected_data_length;
                uint8_t* ps = (uint8_t*)p;
                for (size_t i = 0; i < received_length; ++i) {
                    ps[i] = framein_.data.buff_msg[i];
                }
...
uint8_t _arr[MAX_DATA_LENGTH * 4] = {0};
uint8_t _frame_id = 0;
uint8_t _data_length = 0;
};
struct receive_data_
{
    uint8_t receive_ok[MAX_DATA_LENGTH * 4] = {0};
};

struct
{
    analysis receive;
    std::vector<receive_data_> recevie_datas;
...
void tfCallback(const tf2_msgs::TFMessage::ConstPtr& msg)
{
    for (const auto& transform : msg->transforms)
    {
        ...
        msgs[3] = yaw_deg;
        //mtx.lock();
        ROS_INFO("Position: x=%.6f, y=%.6f, yaw=%.6f", _x, _y, yaw_deg);
...
void mapCallback(const rc_msgs::serialConstPtr& msg)
{
    ...
    uint8_t msgs[msg->map.size()] = {0};
    for (size_t i = 0; i < msg->map.size(); ++i) 
    {
        msgs[i] = (uint8_t)msg->map[i];
    }
    std::lock_guard<std::mutex> lock(global.mtx);
    global.serialComm->serial_send2(msgs, 2, msg->map.size());
}
void pathCallback(const rc_msgs::serialConstPtr& msg)
{
    ROS_INFO("Data array size: %zu", msg->map.size());
    uint8_t msgs[msg->map.size()] = {0};
    for (size_t i = 0; i < msg->map.size(); ++i) 
    ...
    std::lock_guard<std::mutex> lock(global.mtx);
    global.serialComm->serial_send2(msgs, 3, msg->map.size());
}
...
void worker_task2(ros::NodeHandle nh)
{
    ...
    while (ros::ok())
    {
        /* code */
        {
            std::lock_guard<std::mutex> lock(global.mtx_ok);
            if((int)global.recevie_datas[2].receive_ok[0] == 1)
            {
                break;
            }
        }
...
void worker_task3(ros::NodeHandle nh)
{
    ...
    ros::Subscriber map_sub = nh.subscribe("/serial/path", 10, pathCallback);
    while (ros::ok())
    {
        /* code */
        {
            std::lock_guard<std::mutex> lock(global.mtx_ok);
            if((int)global.recevie_datas[3].receive_ok[0] == 1)
            {
                break;
            }
        } 
...
int main(int argc, char **argv)
{
    ...
    global.recevie_datas.resize(10);
    ...
    while(ros::ok())
    {
        ...
        if (global.serialComm->serial_read2(global.receive._arr, global.receive._frame_id, global.receive._data_length)) 
        {
            // if(global.receive._frame_id >= 10)
            // {
            //     status_pub.publish(serial_pub);
            //     continue;
            // }
            //serial_pub.data.push_back(received_data[0]);
            if(global.receive._frame_id == 2 || global.receive._frame_id == 3)
            {

                std::lock_guard<std::mutex> lock(global.mtx_ok);
                if(global.recevie_datas.size() <= global.receive._frame_id)
                {
                    global.recevie_datas.resize(global.receive._frame_id+1);
                }
                for(int i = 0; i < MAX_DATA_LENGTH * 4; i++)
                {
                    global.recevie_datas[global.receive._frame_id].receive_ok[i] = global.receive._arr[i];
                }
                std::cout<<"(int)global.receive._arr[0]:"<<(int)global.receive._arr[0]<<std::endl;
                global.receive.clear();
            }
        }
        status_pub.publish(serial_pub);
