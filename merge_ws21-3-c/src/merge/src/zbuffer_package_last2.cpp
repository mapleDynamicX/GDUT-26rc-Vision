...
public:
    ...
    */ 
    ...
    std::vector<int> get_allow_idxs_(){
        return allow_idxs_;
    }
...
std::unordered_map<int, cv::Mat> Ten_zbuffer::manage_odom_zbuffer_roi(const nav_msgs::Odometry msg,cv::Mat & image)
{
    ...
    if (processed_idxs.size() <= 12) {
        allow_idxs_ = extract_square_hsv_filterEmptyBoxes(
    ...
    std::vector<int> standard_hsv = {result_h,result_s,result_v};
    roi_best_zbuffer(
    image,
    zbuffer,
    allow_idxs_,
...
std::vector<int> Ten_zbuffer::extract_square_hsv_filterEmptyBoxes(
    ...
    //std::cout << "result_idx.size(): " << result_idx.size() << std::endl;
    if (result_idx.size() > 8){
        ...
        //std::cout<< "top8_idx.size(): "<< top8_idx.size() << std::endl;
        // 更新result_idx为前8个结果
        result_idx = top8_idx;
    }
    return result_idx;
...
void Ten_zbuffer::roi_best_zbuffer(
    ...
    for (const auto& [depth, points] : depth_regions) {
        ...
        std::vector<cv::Point> valid_points_f;
        ...
        for (const auto& p : points) {
            ...
            //std::cout << "hsv_score: " << 8 * abs(hsv_value[0] - standard_hsv_[0]) + abs(hsv_value[1] - standard_hsv_[1]) + abs(hsv_value[2] - standard_hsv_[2]) << std::endl;
            if (8 * abs(hsv_value[0] - standard_hsv_[0]) + abs(hsv_value[1] - standard_hsv_[1]) + abs(hsv_value[2] - standard_hsv_[2]) <= _HSV_THRESHOLD_){
                valid_points_f.emplace_back(x, y);
            }
        ...
        int current_count = static_cast<int>(valid_points_f.size());
...
cv::Mat roi_best_zbuffer_debug2(const std::unordered_map<int, cv::Mat>& best_roi_image,const std::vector<int> yolo_result) {
    ...
    for (int idx = 1; idx <= TOTAL_IMGS; ++idx) {
        ...
        if (it != best_roi_image.end() && !it->second.empty()) {
            ...
            // 替换初始化的黑图
            roi_images[vec_idx] = resized_img.clone();
            cv::putText(roi_images[vec_idx], std::to_string(yolo_result[vec_idx]), cv::Point(roi_images[vec_idx].cols - 80 , roi_images[vec_idx].rows - 80), cv::FONT_HERSHEY_SIMPLEX, 1.5, cv::Scalar(0, 255, 0), 2);
        } else {
            ROS_DEBUG("Idx %d no image in best_roi_image, use black image", idx);
        }
    }
