...
public:
    ...
    GroupedROISplitter() {
        // 初始化GROUP_start组的ROI
        // group_rois_[ROI_Group::GROUP_start] = {
        //     {2, cv::Rect(228, 461, 146, 121)},
        //     {3, cv::Rect(691, 457, 140, 127)},
        //     {4, cv::Rect(0, 360, 130, 103)},
        //     {5, cv::Rect(397, 356, 103, 77)},
        //     {7, cv::Rect(173, 305, 96, 83)},
        //     {10, cv::Rect(302, 274, 64, 69)}
        // };

        // // 初始化GROUP_middle组的ROI
        // group_rois_[ROI_Group::GROUP_middle] = {
        //     {4, cv::Rect(0, 782, 305, 288)},
        //     {5, cv::Rect(831, 758, 322, 322)},
        //     {6, cv::Rect(1660, 820, 260, 260)},
        //     {7, cv::Rect(223, 547, 247, 203)},
        //     {8, cv::Rect(913, 586, 159, 159)},
        //     {9, cv::Rect(1529, 556, 235, 204)},
        //     {10, cv::Rect(492, 396, 129, 129)},
        //     {11, cv::Rect(916, 367, 134, 134)},
        //     {12, cv::Rect(1284, 348, 177, 177)}
        // };
        group_rois_[ROI_Group::GROUP_start] = {
            {2, cv::Rect(208, 441, 186, 161)},
            {3, cv::Rect(671, 437, 180, 167)},
            {4, cv::Rect(0, 360, 150, 123)},
            {5, cv::Rect(377, 336, 143, 117)},
            {7, cv::Rect(153, 285, 136, 123)},
            {10, cv::Rect(272, 244, 124, 129)}
        };
        ...
        group_rois_[ROI_Group::GROUP_middle] = {
            ...
            {5, cv::Rect(831, 758, 322, 322)},
            {6, cv::Rect(1660, 820, 260, 260)},
            {7, cv::Rect(213, 537, 267, 223)},
            {8, cv::Rect(913, 586, 159, 159)},
            {9, cv::Rect(1519, 546, 255, 224)},
            {10, cv::Rect(462, 366, 189, 189)},
            {11, cv::Rect(886, 337, 194, 194)},
            {12, cv::Rect(1284, 348, 177, 177)}
        };
    }
...
public:
    ...
    void map_send()
    {
        ...
        for(int i = 0; i < 12; i++)
        {
            serial_pub.map.push_back(0);
        }
        for(int i = 0; i < length; i++)
        {
            serial_pub.map[_han.frames_[i].place - 1] = _han.frames_[i].class_label;
        }
