    ...
    namespace superstratum
    {
        ...
        public:
            ...
            void set_image_for_orb()
            {
                ...
                //设置世界到相机
                transverter_.set_Extrinsic_Matrix(lidar_to_camera_transform_matrix_ * worldtocurrent(lidar_of_world2._xyz, lidar_of_world2._rpy));
                //设置orb推理要素
                Ten::ORB::orb_exhaust_element oee;
                oee.image_ = camera_.camera_read();
                oee.rvec_ = transverter_.rvec();
                oee.tvec_ = transverter_.tvec();
            ...
            bool estimate_square_position()
            {
                ...
                if(losses[0] > 120 || losses[1] - losses[0] < 200)
                {
                    use_relocation();
