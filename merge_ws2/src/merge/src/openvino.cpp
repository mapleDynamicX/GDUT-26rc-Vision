...
public:
    ...
    */
    ...
    cls_thres_(cls_thres)
    {
        ...
        if(model_path == "gpu") compiled_model_ = core_.compile_model(model_, "GPU");
    ...
    std::vector<Detection> postprocess(ov::Tensor& output, int orig_w, int orig_h)
    {
        ...
        if(output_shape_[2] == 8400)
        {
            ...
            for (int i = 0; i < num_detections; ++i) {
                ...
                float cx_ = cx * (float)orig_w /640.0;
                float cy_ = cy * (float)orig_h /640.0;
                float w_ = w * (float)orig_w /640.0;
                float h_ = h * (float)orig_h /640.0;
