...
void test5()
{
    ...
    Ten::Plane_FitLocator::pose_resolving ps(camera.get_color_intrinsics());
    ...
    while (ros::ok())
    {
        ...
        Ten::Plane_FitLocator::Plane_Info pose;
        ps.process(frame, pose, debug);

        cv::imshow("bgr_frame", frame.bgr_image);
        //调试函数可去
...
void test6()
{
    // ==================== ROS1 节点句柄初始化 ====================
    //ros::NodeHandle nh;
    // 2. 创建点云发布者：话题名 /debug_pointcloud，队列长度1
    //ros::Publisher debug_cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("/debug_pointcloud", 1);
    Ten::Plane_FitLocator::Ten_debug_pcl debug_pub_1;
    ...
    while (ros::ok())
    {
        ...
        // ==================== 核心：发布debug点云 ====================
        if (!debug->empty()) // 点云非空时才发布
        {
            // //std::cout << "pub" << std::endl;
            // sensor_msgs::PointCloud2 cloud_msg;
            // // PCL点云 → ROS PointCloud2消息
            // pcl::toROSMsg(*debug, cloud_msg);
            // // 3. 设置坐标系（关键！根据你的相机坐标系修改，如camera_link、camera_depth_optical_frame）
            // cloud_msg.header.frame_id = "map";
            // // 时间戳
            // cloud_msg.header.stamp = ros::Time::now();
            // // 发布点云
            // debug_cloud_pub.publish(cloud_msg);
            debug_pub_1.publish_pointcloud(debug);
            debug_pub_1.publish_PlaneTF(pose);
        }
