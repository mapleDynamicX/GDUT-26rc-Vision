...
void imageCallback(const nav_msgs::Odometry msg, cv::Mat& image_)
{
    ...
    double x =  msg.pose.pose.position.x + 0.0037 + _dx;
    double y =  msg.pose.pose.position.y + 0.0012 + _dy;
    double z =  msg.pose.pose.position.z + 0.0026 + _dz;
    ...
    xyz._z = z;
    Ten::RPY rpy;
    rpy._roll = roll + 0.007 + _droll;
    rpy._pitch = pitch - 0.01 + _dpitch;
    rpy._yaw = yaw - 0.0003 + _dyaw;
