...
import os
# 在 import rospy 之前强行指定 ROS 日志配置文件路径 (针对 Noetic)
...
class TouchscreenUI:
    ...
    def build_grid_block(self, parent, col):
        ...
        # --- 新增：用于固定 6:4 上下比例的主显容器 ---
        split_container = tk.Frame(frame, bg='#2d2d2d')
        split_container.pack(expand=True, fill=tk.BOTH, pady=10)

        # 核心修改：强制上下比例 6:4
        split_container.rowconfigure(0, weight=7, uniform="vert_split")
        split_container.rowconfigure(1, weight=3, uniform="vert_split")
        split_container.columnconfigure(0, weight=1)

        # --- 修改：地图与按钮容器放在第 0 行 (占比 6) ---
        content_container = tk.Frame(split_container, bg='#2d2d2d')
        ...
        path_display_frame = tk.Frame(split_container, bg='#2d2d2d')
        ...
        # 配置 right_frame 的行列权重，使其内部组件可以垂直居中
        right_frame.rowconfigure(0, weight=1)
        right_frame.columnconfigure(0, weight=1)

        # info_container 使用 sticky='w' 靠左，由于 row 权重为 1 且未设置 'ns'，它会垂直居中
        info_container = tk.Frame(right_frame, bg='#2d2d2d')
        info_container.grid(row=0, column=0, sticky='w')
        ...
        tk.Label(info_container, textvariable=self.r2_path_var, fg="#ffff00", bg="#2d2d2d",
                 font=('DejaVu Sans', 20, 'bold'), anchor=tk.W, width=30, justify=tk.LEFT, wraplength=1000).pack(fill=tk.X,
                                                                                                       pady=5)
    ...
    def execute_send_path(self):
        ...
        try:
            ...
            self.path_status_label_1.config(fg="#f1fa8c")  # 恢复黄色准备发送
            self.path_status_var_2.set("")  # 发送时隐藏第二部分
            # ---------------------------
