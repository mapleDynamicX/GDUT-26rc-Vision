...
#include "LIVMapper.h"
std::atomic<bool> _restart_lidar_;
...
// LIVMapper::~LIVMapper()
// {
//     // ========== 第一步：切断所有异步回调入口（最关键，必须放在最前面） ==========
//     // 关停所有订阅者，立即停止接收点云/IMU/图像/重启消息，不再触发新的成员回调
//     std::cout << "close1" << std::endl;
//     sub_pcl.shutdown();
//     sub_imu.shutdown();
//     sub_img.shutdown();
//     restart_lidar.shutdown();

//     // 停止IMU传播定时器，杜绝异步定时回调访问已销毁对象
...
//         lidar_pushed = false;
//         LidarMeasures = LidarMeasureGroup(); // 重置测量组状态
//     }
...
static int receive__ = 0;
static int last__ = 0;

void LIVMapper::lidarrestartCallback(const std_msgs::Int32::ConstPtr& msg)
{
  //ROS_INFO("callback entered, msg ptr = %p", msg.get());
  receive__ = msg->data;
  if(receive__ != last__)
  {
      last__ = receive__;
      _restart_lidar_.store(true);
      response(last__);
  }
  else
  {
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  sub_pcl = p_pre->lidar_type == AVIA ? 
            nh.subscribe(lid_topic, 2000, &LIVMapper::livox_pcl_cbk, this): 
            nh.subscribe(lid_topic, 2000, &LIVMapper::standard_pcl_cbk, this);
  sub_imu = nh.subscribe(imu_topic, 2000, &LIVMapper::imu_cbk, this);
  sub_img = nh.subscribe(img_topic, 2000, &LIVMapper::img_cbk, this);
  restart_lidar = nh.subscribe("/scripts/restart_lidar2", 1, &LIVMapper::lidarrestartCallback, this);
  
  pubLaserCloudFullRes = nh.advertise<sensor_msgs::PointCloud2>("/fast_lio2/cloud_registered", 10);
  pubNormal = nh.advertise<visualization_msgs::MarkerArray>("/fast_lio2visualization_marker", 1);
  pubSubVisualMap = nh.advertise<sensor_msgs::PointCloud2>("/fast_lio2/cloud_visual_sub_map_before", 1);
  pubLaserCloudEffect = nh.advertise<sensor_msgs::PointCloud2>("/cloud_effected", 1);
  pubLaserCloudMap = nh.advertise<sensor_msgs::PointCloud2>("/fast_lio2/feats_undistort", 10);
  pubOdomAftMapped = nh.advertise<nav_msgs::Odometry>("/fast_lio2/odom", 10);
  pubPath = nh.advertise<nav_msgs::Path>("/fast_lio2/path", 1);
  plane_pub = nh.advertise<visualization_msgs::Marker>("/fast_lio2/planner_normal", 1);
  voxel_pub = nh.advertise<visualization_msgs::MarkerArray>("/fast_lio2/voxels", 1);
  pubLaserCloudDyn = nh.advertise<sensor_msgs::PointCloud2>("/fast_lio2/dyn_obj", 1);
  pubLaserCloudDynRmed = nh.advertise<sensor_msgs::PointCloud2>("/fast_lio2/dyn_obj_removed", 1);
  pubLaserCloudDynDbg = nh.advertise<sensor_msgs::PointCloud2>("/fast_lio2/dyn_obj_dbg_hist", 1);
  mavros_pose_publisher = nh.advertise<geometry_msgs::PoseStamped>("/fast_lio2/mavros/vision_pose/pose", 1);
  pubImage = it.advertise("/fast_lio2/rgb_img", 1);
  pubImuPropOdom = nh.advertise<nav_msgs::Odometry>("/fast_lio2/LIVO2/imu_propagate", 1);
  imu_prop_timer = nh.createTimer(ros::Duration(0.004), &LIVMapper::imu_prop_callback, this);
  voxelmap_manager->voxel_map_pub_= nh.advertise<visualization_msgs::MarkerArray>("/fast_lio2/planes", 1);
...
{    
  ...
  //发布没有畸变的点云
  //publishPointCloud(pubLaserCloudMap, feats_undistort);
...
void LIVMapper::publish_odometry(const ros::Publisher &pubOdomAftMapped)
{
  ...
  br.sendTransform( tf::StampedTransform(transform, odomAftMapped.header.stamp, "camera_init", "fast_lio2") );
...
/**
 * @brief 适配 FAST-LIVO2 原生点云的通用发布函数
 * @param pub ROS 发布者
 * @param cloud 输入点云 (PointCloudXYZI::Ptr, 代码原生类型)
 * @param frame_id 坐标系
 */
void LIVMapper::publishPointCloud(ros::Publisher& pub, const PointCloudXYZI::Ptr& cloud, const std::string& frame_id)
{
  // 空点云直接返回，防止崩溃
...
void LIVMapper::reset()
{
    ...
    sync_jump_flag = false;
    flg_reset = false;

    last_timestamp_lidar = -1.0;
    last_timestamp_imu = -1.0;
    last_timestamp_img = -1.0;
    _first_lidar_time = 0.0;

    frame_num = 0;
    aver_time_consu = 0;
    aver_time_icp = 0;
    aver_time_map_inre = 0;

    // ========== 3. 重置核心算法状态 ==========
    _state = StatesGroup();
    state_propagat = StatesGroup();
    imu_propagate = StatesGroup();
    latest_ekf_state = StatesGroup();
    ...
    std::cout << "[RESET] LIVMapper state reset finished." << std::endl;
}
