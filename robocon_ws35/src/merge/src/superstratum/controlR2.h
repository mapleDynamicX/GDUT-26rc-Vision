        ...
        static void serial_send_lidarR2_ekf_imu_withprotected()
        {
            ...
            while(Ten::_TREADPOOL_FLAG_.read_flag())
            {
                ...
                while(Ten::_POINT_LIO_RUN_FLAG_.read_flag() && Ten::_TREADPOOL_FLAG_.read_flag())
                {
                    ...
                    //pose = Ten::math::transform_matrixtoXYZRPY_fixed(Ten::math::XYZRPYtotransform_matrix_fixed(xyzrpy_errorx)*Ten::math::XYZRPYtotransform_matrix_fixed(pose));
                    ...
                    xyzrpy_errorx._rpy._yaw = _r2_xyzrpy_init_error_rpy_yaw_.load();
        ...
        static void serial_receiver()
        {
            ...
            while(Ten::_TREADPOOL_FLAG_.read_flag())
            {
                ...
                if(serial.serial_read2(arr, frame_id, length))
                {
                    ...
                    if(frame_id == 2)
                    {
                        // 正确拼接：高字节左移8位，按位或低字节，先拼无符号再转有符号
                        data_liao_serial[0] = (int16_t)((int16_t)arr[0] << 8 | arr[1]);
                        data_liao_serial[1] = (int16_t)((int16_t)arr[2] << 8 | arr[3]);
                        data_liao_serial[2] = (int16_t)((int16_t)arr[4] << 8 | arr[5]);
                        data_liao_serial[3] = (int16_t)((int16_t)arr[6] << 8 | arr[7]);
                        data_liao_serial[4] = (int16_t)((int16_t)arr[8] << 8 | arr[9]);

                        for(size_t i = 0; i < 5; i++)
                        {
                            if(i < 3)
                            {
                                // x/y/yaw：发送端*100，接收端/100还原，逻辑正确
                                data_liao[i] = static_cast<float>(data_liao_serial[i]) / 100.0f;
                            }
                            else
                            {
                                // 距离值：直接保留整数，逻辑正确
