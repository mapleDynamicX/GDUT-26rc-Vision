...
public:
    ...
    */
    ...
    Ten::XYZRPY get_transformation(typename PointCloudT::Ptr local_cloud)
    {
        ...
        voxelDownSample(global_cloud_, global_cloud_downsampled, 0.25); // 体素0.05m，可根据点云密度调整
        voxelDownSample(local_cloud_, local_cloud_downsampled, 0.25);
        ...
        params.noise_bound = 0.25;          // 噪声边界（与添加的噪声一致）
        ...
        Eigen::Matrix4d inverse_transform = transform_matrix_raw.inverse();
...
private:
    ...
    const FPFHCloudT::Ptr& src_fpfh, const FPFHCloudT::Ptr& tgt_fpfh, pcl::Correspondences& correspondences) 
    {
        ...
        ce.determineCorrespondences(correspondences, 6); // 距离阈值，可调整
     ...
     */
    Eigen::Matrix4d icpRegistration(const typename PointCloudT::Ptr& source_cloud, const typename PointCloudT::Ptr& target_cloud)
    {
        ...
        // 3. 设置ICP输入点云
        icp.setInputSource(source_cloud);  // 待配准的源点云
        icp.setInputTarget(target_cloud);  // 参考的目标点云
