...
//#define ROOT_DIR "/home/maple/study2/lidar_ws2/src/Point-LIO/"
 ...
 */
void voxelDownSample(const pcl::PointCloud<pcl::PointXYZI>::Ptr input_cloud, pcl::PointCloud<pcl::PointXYZI>::Ptr& output_cloud, float leaf_size = 0.1) 
{
...
void publish_frame_world(const ros::Publisher & pubLaserCloudFullRes)
{
    if (scan_pub_en)
    {
        ...
        second->reserve(100000);
        //*global_cloud_downsampled += *laserCloudWorld;
        //*tmp = *global_cloud_downsampled;
        statisticalFilter(laserCloudWorld, first);
        *global_cloud_downsampled += *first;
        voxelDownSample(global_cloud_downsampled, second, _voxeldownsample_threshold_);
        if(second->size() >= 100000)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                global_cloud_downsampled->points[i].z = second->points[i*2].z;
                global_cloud_downsampled->points[i].intensity = second->points[i*2].intensity; 
            }
        }
        else
        {
            global_cloud_downsampled = second;
        }
        //global_cloud_downsampled = second;
...
void set_odometry(T & out)
{
    if (!use_imu_as_input)
    {
        ...
        // out.twist.twist.angular.x = input_in.gyro(0);
        // out.twist.twist.angular.y = input_in.gyro(1);
        // out.twist.twist.angular.z = input_in.gyro(2);   
