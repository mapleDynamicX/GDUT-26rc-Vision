...
namespace Ten
{
    ...
    namespace Tencctag
    {
        #define _proportion_1_2_ 0.517   //_IDX_0_指向_IDX_1_， 然后取比例为_proportion_1_2_
        #define _length_of_offset_ -0.002   //距离_proportion_1_2_计算出线上的点，并垂直与直线向上的距离
        #define _length_of_pole_ 0.067 //杆的长度
        ...
        public:
            ...
            int getpose(cv::Mat& image, double& biasx, double& biasy, Ten::XYZRPY& pose)
            {
                ...
                //获取检测结果
                std::vector<cctagresult> rets = rel_pose_.resolver(resize);
