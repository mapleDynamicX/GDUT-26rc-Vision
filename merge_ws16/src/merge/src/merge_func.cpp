...
void serial_send_lidar()
{
    ...
    Ten::_COORDINATE_TRANSFORMATION_.set_stead_state_error(xyzrpy_car);    
    ...
    while(ros::ok())
    {
        ...
        arr[6] = odo.twist.twist.linear.x;
        arr[7] = odo.twist.twist.linear.y;
        arr[8] = odo.twist.twist.linear.z;
