#ifndef __YOLO_HAN2_H_
#define __YOLO_HAN2_H_
...
namespace Ten
{
    ...
    namespace yolo
    {
        ...
        public:
            ...
            */
            yolo_han2(const std::string model_path, const std::string xpu)
            {
                core_ = ov::Core();
                ...
                if(output_shape_.size() != 3 || output_shape_[1] != 12 || output_shape_[2] != 2)
                {
                    flag_ = 0;
                }
            }
            ...
            */
            virtual std::vector<han2> worker(std::vector<cv::Mat>& img) 
            {
                if(flag_ == 0)
                {
                    std::cout << "😡 模型输出不匹配" << std::endl;
                    return std::vector<han2>();
                }
                if(img.size() != 12)
                {
        ...
        protected:
            ...
            */
            ...
            virtual std::vector<han2> postprocess(ov::Tensor& output) 
            {
                std::vector<han2> detections;
                ...
                for (int i = 0; i < num_image; ++i) {
                    ...
                    han2 h;
                    h.valid_empty_ = channels[i][0];
                    h.valid_exist_ = channels[i][1];
