...
class TouchscreenUI:
    ...
    def build_param_block_content(self, parent):
        ...
        # 保存 Canvas 窗口项 ID，避免后续 find_all 出错
        ...
        # 单个参数块的固定宽度（像素）
        self.param_block_width = 600
        # 参数块之间的横向间距（像素）
        self.param_block_gap = 20
        ...
        # 【关键修复】初始化完成后手动触发一次重排，确保首次加载就能显示
    ...
    def _create_single_param_widget(self, parent_container, param_key):
        ...
        # 【修复】给参数块设置最小高度，避免内容未渲染时高度为0
        block = tk.Frame(
            parent_container, bg="#383838", bd=2, relief=tk.RAISED,
            width=self.param_block_width, height=300
        )
        ...
        for val in range(minv, maxv + 1):
            ...
            btn = tk.Button(
                btn_wrap, text=txt, font=("DejaVu Sans", 60, "bold"),
    ...
    def _on_param_canvas_resize(self, event):
        """画布宽度变化时，自动计算列数并重排所有参数块"""
        canvas_width = event.width
        if canvas_width <= 1:
            return  # 宽度异常时跳过，避免初始渲染报错

        # 计算一行可容纳的参数块数量（至少1列）
        cols = max(1, (canvas_width + self.param_block_gap) // (self.param_block_width + self.param_block_gap))
