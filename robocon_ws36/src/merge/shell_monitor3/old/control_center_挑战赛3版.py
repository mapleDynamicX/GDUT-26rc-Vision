...
class TouchscreenUI:
    ...
    def build_grid_block_content(self, parent):
        ...
        tk.Label(info_container, textvariable=self.r2_path_var, fg="#000000", bg="#2d2d2d",
                 font=('DejaVu Sans', 30, 'bold'), anchor=tk.W, width=35, justify=tk.LEFT, wraplength=700).pack(
            fill=tk.X, pady=5)
        tk.Label(info_container, textvariable=self.r1_path_var, fg="#0021FA", bg="#2d2d2d",
                 font=('DejaVu Sans', 30, 'bold'), anchor=tk.W, width=35, justify=tk.LEFT, wraplength=1200).pack(
            fill=tk.X, pady=5)
    ...
    def _update_manual_text(self):
        """刷新UI上R1和R2文本的显示"""
        r2_str = " → ".join([str(i + 1) for i in self.r2_robot_path]) if self.r2_robot_path else "未选择"
        r1_str = " → ".join([str(i + 1) for i in self.r1_targets]) if self.r1_targets else "未选择"
        self.r2_path_var.set(f"R2 路径：{r2_str}")
