...
class TouchscreenUI:
    ...
    def run_data_monitor_script(self):
        ...
        try:
            ...
            cmd = ["gnome-terminal", "--", "bash", "-c", f"bash {sh_script_path}; exec bash"]
            ...
            # 如果你有底部的状态提示栏，可以反注释并修改这一行
            # self.script_status_var.set("[已运行] 数据监视终端已启动")

        except Exception as e:
            rospy.logerr(f"启动数据监视脚本失败: {e}")
            # messagebox.showerror("错误", f"启动数据监视脚本失败:\n{e}")
    ...
    def _build_plan_table(self):
        ...
        tk.Label(self.table_frame, text="R1\\R2", bg=bg_transparent, fg="black",
                 font=('DejaVu Sans', 15, 'bold'), width=7).grid(row=0, column=0, sticky='nsew', pady=1)

        for col_idx, r2_cnt in enumerate(r2_range, start=1):
            tk.Label(self.table_frame, text=f"{r2_cnt}个", bg=bg_transparent, fg="black",
                     font=('DejaVu Sans', 15, 'bold')).grid(row=0, column=col_idx, sticky='nsew', padx=1, pady=1)

        for row_idx, r1_cnt in enumerate(r1_range, start=1):
            tk.Label(self.table_frame, text=f"{r1_cnt}个", bg=bg_transparent, fg="black",
                     font=('DejaVu Sans', 15, 'bold')).grid(row=row_idx, column=0, sticky='nsew', padx=1, pady=1)
        # ==========================================================
            for col_idx, r2_cnt in enumerate(r2_range, start=1):
                lbl = tk.Label(self.table_frame, text="-", bg="#44475a", fg="black", width=5,
    ...
    def build_grid_block_content(self, parent):
        ...
        self.manual_btn_path = tk.Button(self.manual_tools_frame, text="路径", font=('DejaVu Sans', 25, 'bold'),
                                         command=lambda: self.set_manual_edit_type("path"))
        self.manual_btn_r1 = tk.Button(self.manual_tools_frame, text="R1", font=('DejaVu Sans', 25, 'bold'),
                                       command=lambda: self.set_manual_edit_type("r1"))
        self.manual_btn_r2 = tk.Button(self.manual_tools_frame, text="R2", font=('DejaVu Sans', 25, 'bold'),
        ...
        btn_top_frame.grid(row=0, column=0, sticky='ne', padx=5, pady=5)

        self.manual_mode_btn = tk.Button(btn_top_frame, text="切换手动路径", bg="#44475a", fg="white",
                                         font=('DejaVu Sans', 30, 'bold'), command=self.toggle_manual_mode)
        self.manual_mode_btn.pack(side=tk.LEFT, padx=5)

        self.court_toggle_btn = tk.Button(btn_top_frame, text="切换为蓝场", font=('DejaVu Sans', 30, 'bold'),
                                          command=self.toggle_court_side)
        self.court_toggle_btn.pack(side=tk.LEFT, padx=5)
        # 新增：赛制切换按钮
        self.match_mode_toggle_btn = tk.Button(btn_top_frame, text="切换挑战赛", font=('DejaVu Sans', 30, 'bold'),
        ...
        self.r2_path_label = tk.Label(info_container, textvariable=self.r2_path_var, fg="#000000", bg="#2d2d2d",
                 font=('DejaVu Sans', 30, 'bold'), anchor=tk.W, width=35, justify=tk.LEFT, wraplength=700)
        self.r2_path_label.pack(fill=tk.X, pady=5)
        self.r1_path_label = tk.Label(info_container, textvariable=self.r1_path_var, fg="#0021FA", bg="#2d2d2d",
                 font=('DejaVu Sans', 30, 'bold'), anchor=tk.W, width=35, justify=tk.LEFT, wraplength=1200)
        self.r1_path_label.pack(fill=tk.X, pady=5)
    ...
    def build_ros_command_block(self, parent, col):
        ...
        monitor_btn.pack(fill=tk.BOTH, expand=True, pady=10)
        # ============================================================
    ...
    def execute_send_path(self):
        ...
        if getattr(self, 'is_manual_mode', False):
            ...
            # 获取R2格式化好的文本（去除前缀）
            r2_list = self.r2_path_var.get().replace("R2 路径：", "").strip()
