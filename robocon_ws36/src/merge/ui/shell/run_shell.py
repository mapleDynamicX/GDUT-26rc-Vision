...
class ScriptRunnerUI:
    def __init__(self, master):
        ...
        self.master.title("运行")
        self.master.geometry(f"{win_width}x{win_height}+{x_coordinate}+{y_coordinate}")
        self.master.configure(bg='#1e1e1e')

        # 自定义一个超高（高度 60 像素）的标题栏
        self.custom_title_bar = tk.Frame(self.master, bg='#2d2d2d', height=50)
        self.custom_title_bar.pack(side=tk.TOP, fill=tk.X)
        self.custom_title_bar.pack_propagate(False) # 锁定高度不变形

        # 标题栏文字
        title_label = tk.Label(self.custom_title_bar, text="脚本运行面板", bg='#2d2d2d', fg='white', font=('DejaVu Sans', 12, 'bold'))
        title_label.pack(side=tk.LEFT, padx=20)

        # 超大尺寸的红色关闭按钮（24号加粗字体，填满高度）
        close_btn = tk.Button(self.custom_title_bar, text=" ✕ ", bg='#ff5555', fg='white', 
                              font=('DejaVu Sans', 12, 'bold'), bd=0, command=self.on_closing,
                              activebackground='#ff3333', activeforeground='white')
        close_btn.pack(side=tk.RIGHT, fill=tk.Y)

        # 绑定拖动事件，让无边框窗口也能用鼠标拖动
        # self.custom_title_bar.bind("<Button-1>", self.start_drag)
        # self.custom_title_bar.bind("<B1-Motion>", self.do_drag)
        # title_label.bind("<Button-1>", self.start_drag)
        # title_label.bind("<B1-Motion>", self.do_drag)

        # ========== 状态变量 ==========
        self.selected_script_info = None  
        self.script_buttons = []          
    ...
    def do_drag(self, event):
        """跟随鼠标拖动更新整个窗口的位置"""
        x = self.master.winfo_x() + (event.x - self.x_offset)
        y = self.master.winfo_y() + (event.y - self.y_offset)
    ...
    def build_main_ui(self):
        ...
        status_label = tk.Label(main_frame, textvariable=self.script_status_var,
                                bg='#1e1e1e', fg='#50fa7b', font=('DejaVu Sans Mono', 8), height=2)
    ...
    def load_and_create_script_buttons(self):
        ...
        for s in scripts:
            ...
            btn = tk.Button(self.script_container, text=name, bg="#44475a", fg="white",
                            font=('DejaVu Sans', 15), height=2, anchor="w")
            btn.pack(fill=tk.X, padx=15, pady=4, expand=True)
