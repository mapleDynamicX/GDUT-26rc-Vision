...
class ScriptRunnerUI:
    ...
    def __init__(self, master):
        ...
        self.custom_title_bar = tk.Frame(self.master, bg='#2d2d2d', height=90)
        self.custom_title_bar.pack(side=tk.TOP, fill=tk.X)
        self.custom_title_bar.pack_propagate(False) # 锁定高度不变形

        # 标题栏文字
        title_label = tk.Label(self.custom_title_bar, text="脚本运行面板", bg='#2d2d2d', fg='white', font=('DejaVu Sans', 16, 'bold'))
        title_label.pack(side=tk.LEFT, padx=20)

        # 超大尺寸的红色关闭按钮（24号加粗字体，填满高度）
        close_btn = tk.Button(self.custom_title_bar, text=" ✕ ", bg='#ff5555', fg='white', 
                              font=('DejaVu Sans', 16, 'bold'), bd=0, command=self.on_closing,
    ...
    def build_main_ui(self):
        ...
        self.control_bar.pack(side=tk.TOP, fill=tk.X, padx=1, pady=1)
        ...
        btn_run_term.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=3, pady=3)

        btn_stop = tk.Button(inner_bar, text="停止", bg="#44475a", fg="white",
                             font=('DejaVu Sans', 25, 'bold'),
                             command=self.stop_selected_script)
        btn_stop.pack(side=tk.LEFT, expand=True, fill=tk.X, pady=3)

        # 2. 滚动脚本列表区域
        list_container = tk.Frame(main_frame, bg='#2d2d2d')
        list_container.pack(expand=True, fill=tk.BOTH, padx=3, pady=5)
