...
namespace Ten
{
    ...
    public:
        ...
        void set_stead_state_error(Ten::XYZRPY stead_state_error)
        {
            ...
            pitch_ = stead_state_error._rpy._pitch;
        }  
        ...
        {
            std::lock_guard<std::mutex> lock(mtx_);
            if(std::fabs(pitch_ - worldtolidar_._rpy._pitch) < 0.02)
            {
                stead_state_error_._rpy._pitch = worldtolidar_._rpy._pitch;
            }
            else
            {
                stead_state_error_._rpy._pitch = pitch_;
            }
            Eigen::Matrix4d T1 = XYZRPYtotransform_matrix(lidartocar_);
            Eigen::Matrix4d T1_N = T1.inverse();
